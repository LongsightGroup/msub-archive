...
public class JLDAPDirectoryProvider implements UserDirectoryProvider, LdapConnectionManagerConfig, ExternalUserSearchUDP
{
	...
	/** LDAP connection port. Defaults to {@link #DEFAULT_LDAP_PORT} */
	private int secondaryLdapPort = DEFAULT_LDAP_PORT;
	
	/** LDAP host address */
	private String tertiaryLdapHost;

	/** LDAP connection port. Defaults to {@link #DEFAULT_LDAP_PORT} */
	private int tertiaryLdapPort = DEFAULT_LDAP_PORT;
	...
	public void init()
	{
		...
		if (StringUtils.isNotEmpty(ldapDomain)) {
			...
			if ( M_log.isDebugEnabled() ) {
				M_log.debug("init(): compiled a pattern for domain checking: " + patternString);
			}
		}
	...
	public boolean findUserByEmail(UserEdit edit, String email)
	{
		if ( !isSearchableEid(email) ) {
			M_log.debug("findUserByEmail(): passing on search because of domain [email = " + email + "].");
			return false;
		}
	...
	public void getUsers(Collection<UserEdit> users)
	{
		if ( M_log.isDebugEnabled() ) {
			if (StringUtils.isNotEmpty(ldapDomain)) {
				M_log.debug("getUsers(): [Collection size = " + users.size() + ";domain=" + ldapDomain + "]");
			}
			else {
		...
		try {
			...
			for ( Iterator<UserEdit> userEdits = users.iterator(); userEdits.hasNext(); ) {
				...
				// Do not search for this user if not in domain or is blacklisted
				...
				if (M_log.isDebugEnabled()) {
					M_log.debug("usersToSearchInLDAP: " + usersToSearchInLDAP.size() + ";eid=" + eid + ";hasNext=" + userEdits.hasNext() + ";cnt=" + cnt);
				}
				...
				if ((!userEdits.hasNext() || cnt == maxQuerySize) && !usersToSearchInLDAP.isEmpty()) {
					...
					for (LdapUserData ldapUserData : ldapUsers) {
						String ldapEid = ldapUserData.getEid().toLowerCase();
						UserEdit ue = usersToSearchInLDAP.get(ldapEid);

						if (StringUtils.isNotBlank(ldapEid) && ue != null) {
	...
	throws LDAPException {
		...
		if ((loginKey.equals("samaccountname") || loginKey.equals("uid")) && StringUtils.contains(eid, "@")) {
			filter = "sAMAccountName=" + StringUtils.substringBefore(eid, "@");
			
			if (M_log.isDebugEnabled()) {
				M_log.debug("Changed the filter because of use of sAMAccountName: " + filter);
			}
		}
	...
	protected boolean isSearchableEid(String eid) {
		if (domainPattern != null) {
			if (domainPattern.matcher(eid).matches()) {
				if ( M_log.isDebugEnabled() ) {
					M_log.debug("domainPattern rejected: " + domainPattern.pattern() + "::" + eid);
				}
				return false;
			}
		}
	...
	throws LDAPException {
		...
		try {
			...
			if (conn == null) {
				M_log.warn("JLDAP no valid connections available for a searchDirectory call");
	...
	protected LdapUserData getCachedUserEntry(String eid) {
		// depend solely on the UserDirectoryService central cache
		return null;
	}
	...
	protected void cacheUserData(LdapUserData user){
		// let UserDirectoryService do the caching
	}
	...
	{
		return secondaryLdapHost;
	}

	/**
	 * {@inheritDoc}
	 */
	public void setSecondaryLdapHost(String ldapHost)
	{
		this.secondaryLdapHost = ldapHost;
	}

	/**
	 * {@inheritDoc}
	 */
	public int getSecondaryLdapPort()
	{
		return secondaryLdapPort;
	}

	/**
	 * {@inheritDoc}
	 */
	public void setSecondaryLdapPort(int ldapPort)
	{
		this.secondaryLdapPort = ldapPort;
	}
	
	/**
	 * {@inheritDoc}
	 */
	public String getTertiaryLdapHost()
	{
		return tertiaryLdapHost;
	}

	/**
	 * {@inheritDoc}
	 */
	public void setTertiaryLdapHost(String ldapHost)
	{
		this.tertiaryLdapHost = ldapHost;
	}

	/**
	 * {@inheritDoc}
	 */
	public int getTertiaryLdapPort()
	{
		return tertiaryLdapPort;
	}

	/**
	 * {@inheritDoc}
	 */
	public void setTertiaryLdapPort(int ldapPort)
	{
		this.tertiaryLdapPort = ldapPort;
	}
	...
	public List<UserEdit> searchExternalUsers(String criteria, int first, int last, UserFactory factory) {
		...
		try {
			String domain = getLdapDomain().toLowerCase();

			if (StringUtils.isNotEmpty(domain)) {
					User currentUser = userDirectoryService.getCurrentUser();
					String email = currentUser.getEmail().toLowerCase();

					if (StringUtils.isNotEmpty(email) && StringUtils.isNotEmpty(domain) && StringUtils.contains(email, "@")) {
							if (M_log.isDebugEnabled()) {
									M_log.debug("Checking if current user (" + email +") should be searching against this domain (" + domain + ")");
							}
			...
			M_log.warn("searchExternalUsers(): Error trying to match on domain", e);
		}
