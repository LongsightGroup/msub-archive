/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/msub/longsight.com/providers/sakai-19.x_jldap-auth-caching-negative-cache/jldap/src/java/edu/amc/sakai/user/JLDAPDirectoryProvider.java $
 * $Id: JLDAPDirectoryProvider.java 326991 2019-10-04 17:28:38Z ottenhoff@longsight.com $
 ***********************************************************************************
 *
						...
						String ldapEid = ldapUserData.getEid();
						        ...
						        UserEdit ue = usersToSearchInLDAP.get(ldapEid);
						                ...
						                mapUserDataOntoUserEdit(ldapUserData, ue);
                                                                ...
                                                                M_log.warn("getUsers: LDAP results contain a user we didn't ask for: " + ldapEid);
                                                        }
						        usersToSearchInLDAP.remove(ldapEid);
                                                }
                                                else {
                                                        if (M_log.isDebugEnabled()) {
                                                                M_log.debug("usersToSearchInLDAP: null user: " + eid);
                                                        }
                                                }
					}
	...
	public List<UserEdit> searchExternalUsers(String criteria, int first, int last, UserFactory factory) {
		...
		try {
			...
			for(LdapUserData ldapUserData: ldapUsers) {
				...
				UserEdit user = factory.newUser(ldapUserEid);
