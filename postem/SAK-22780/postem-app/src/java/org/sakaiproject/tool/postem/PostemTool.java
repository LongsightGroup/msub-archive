...
public class PostemTool {
	...
	public String processCreate() {
		...
		if (attachment == null){			
			//only fail if this was a brand new gradebook
			
			if (currentGradebook.getId() != null) {
				this.csv = this.createCSVFromGradebook(currentGradebook); //generate the CSV from the existing gradebook
			} else {
				return "create_gradebook";
			}
		}
		
		if (this.delimiter != null && !this.delimiter.equals(COMMA_DELIM_STR) && !this.delimiter.equals(TAB_DELIM_STR)) {
	...
	public String processCsvDownload() {
		...
		this.csv = createCSVFromGradebook(currentGradebook);
		return "download_csv";
	}
	...
	private String createCSVFromGradebook(Gradebook gb) {
		List csvContents = new ArrayList();
		if (gb.getHeadings().size() > 0) {
			csvContents.add(gb.getHeadings());
		}
		Iterator si = gb.getStudents().iterator();
		while (si.hasNext()) {
		...
		CSV newCsv = new CSV(csvContents, gb.getHeadings().size() > 0);
		
		return newCsv.getCsv();
	}
