...
public class MySession implements Session, HttpSession, Serializable
{
	...
	{
        if (LOG.isDebugEnabled()) {
            LOG.debug("isInactive() for sessionid:" + m_id + " => m_inactiveInterval=" + m_inactiveInterval + ",System.currentTimeMillis()="+ System.currentTimeMillis() +
                    ",m_accessed=" + m_accessed + ",m_inactiveInterval=" + m_inactiveInterval);
            LOG.debug(" isInactive() for sessionid:" + m_id + " => " + ((m_inactiveInterval > 0) && (System.currentTimeMillis() > (m_accessed + (m_inactiveInterval * 1000)))) );
