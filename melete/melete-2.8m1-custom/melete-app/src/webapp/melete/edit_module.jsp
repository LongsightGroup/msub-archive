...
var groupPanelDisplaying = true;
function toggleGroupPanel(){
	var groupPanel = document.getElementById('EditModuleForm:groupAccess:groupPanel');
	if(groupPanelDisplaying){
		groupPanel.style.display="none";
	}else{
		groupPanel.style.display="";
	}
	groupPanelDisplaying = !groupPanelDisplaying;
	
	setMainFrameHeight('<h:outputText value="#{editModulePage.winEncodeName}"/>');
}
	...
	<table class="maintableCollapseWithBorder" id="AutoNumber1" >
         <tr>
          <td valign="top">
              ...
              <tr>
                <td class="col1" align="left" valign="top"> <h:outputText rendered="#{editModulePage.showSubtitles}" value="#{msgs.edit_module_module_subtitle}" /> </td>
                <td class="col2" align="left" valign="top">
                  <h:inputText id="subtitle" size="45" rendered="#{editModulePage.showSubtitles}" value="#{editModulePage.module.subtitle}" styleClass="formtext" />
                </td>
              </tr>
          ...
          <h:outputLink id="viewedateCal" onclick="showEdateCal()" value="#endCalender">
            <h:graphicImage id="edateCal"  value="images/date.png"  alt="#{msgs.list_auth_modules_alt_popup_cal}" title="#{msgs.list_auth_modules_alt_popup_cal}" styleClass="DatePickerClass"/>
              ...
              <f:subview id="groupAccess" rendered="#{editModulePage.siteHasGroups}">
	              <f:verbatim>
	              <tr>
	                <td  class="col1" align="left" valign="top">
	              </f:verbatim>
	                <h:outputText value="#{msgs.edit_module_group_access}"/>
	              <f:verbatim>
	              	</td>              
	                <td  class="col2" valign="top">
	              </f:verbatim>
	                 <h:selectBooleanCheckbox id="groupRestrictionFlag" title="groupRestrictionFlag" value="#{editModulePage.module.groupRestrictionFlag}" onchange="toggleGroupPanel();">
			         </h:selectBooleanCheckbox>
			         <h:outputText id="groupRestrictionFlagTxt" value="#{msgs.edit_module_group_restriction}"/>
			      <f:verbatim>
			      		<br/>
			      		<br/>
			      		</f:verbatim>
			      		
			      		<h:panelGroup id="groupPanel" styleClass="shorttext">
			      			<f:verbatim>
			      				<div>
			      			</f:verbatim>
					    	<h:outputText value="#{msgs.edit_module_group_add}: "/>
					    	<h:selectOneListbox size="1" id="availableGroup" value ="#{editModulePage.selectedAvailableGroup}" onchange="this.form.submit();"
								                  valueChangeListener="#{editModulePage.processActionAddGroup}">
						      <f:selectItems value="#{editModulePage.availableGroups}"/>
						    </h:selectOneListbox>
						    
						    <h:dataTable id="table"  value="#{editModulePage.accessGroups}"   styleClass="listHier lines nolines" style="width: auto" var="accessGroup"  border="0" rendered="#{!empty editModulePage.accessGroups}">
							  <h:column>
								   <f:facet name="header">
										<h:panelGroup>
										  <h:outputText value="#{msgs.edit_module_groups}" />
										 </h:panelGroup> 
									 </f:facet>
  				
			  						<h:outputText value="#{accessGroup.groupTitle}"/>
			  						<h:commandLink action="#{editModulePage.processActionRemoveGroup}">
						      			<f:param value="#{accessGroup.groupId}" name="groupId"/>
						      			<h:graphicImage url="/images/cross.png" title="#{msgs.edit_module_group_remove}" alt="#{msgs.edit_module_group_remove}" style="margin-left:.5em"/>
						      		</h:commandLink>
			  					</h:column>
			  				</h:dataTable>
			  				<f:verbatim>
			  				...
			  				<h:outputText value="#{msgs.edit_module_no_groups}" styleClass="BlueClass" rendered="#{empty editModulePage.accessGroups}"/>
			  				<f:verbatim>
			      				</div>
			      			</f:verbatim>
					    </h:panelGroup> 		
			      		<f:subview id="siteHideGroupsView" rendered="#{!editModulePage.module.groupRestrictionFlag}">
							<f:verbatim>
