...
public class ListModulesPage implements Serializable{
	/** Dependency:  The logging service. */
      ...
      private Boolean showInactiveModulesMaterial = null;
      private boolean printable;
      private boolean autonumber;
      private boolean showInactiveModules;
...
//	  	context.getViewRoot().setTransient(true);
	  ...
	  public void resetValues()
	  {
	  	...
	  	closedModulesFlag = false;
	  	printMaterial = null;
	  	showInactiveModulesMaterial = null;
 ...
 /**
	  ...
	  */
      ...
      public boolean getShowSubtitles() {
          String showSubtitles = ServerConfigurationService.getString("melete.showSubtitles", "false");
          return Boolean.parseBoolean(showSubtitles);
      }
	  ...
	  public List getViewModuleBeans() {
	  	    ...
	  	    catch (Exception e)
		    {
	  	    ...
	  	    //filter any group modules the user doesn't have access to
	  	    if (viewModuleBeans != null && getRole()!= null && getRole().equals("STUDENT")){
	  	    	for (ListIterator i = viewModuleBeans.listIterator(); i.hasNext();)
	  	    	{
	  	    		...
	  	    		if (vmbean.getGroupRestrictionFlag()){
	  	    			if(!isUserAMemberOfAnyGroups(vmbean.getAccessGroups())){
	  	    				i.remove();
	  	    			}
	  	    		}
	  	    	}
	  	  ...
	  	  if (viewModuleBeans != null && getRole()!= null && getRole().equals("STUDENT") && !isShowInactiveModules()){
	  		  //fileter out inactive modules in student view:
	  		for (ListIterator i = viewModuleBeans.listIterator(); i.hasNext();)
			{
				ViewModBean vmbean = (ViewModBean) i.next();
				if (vmbean.isVisibleFlag() == false){
					//this isn't visible and prefs don't want to show inactive modules, so remove
					i.remove();
				}
			}
	  	  }	  			  
	  ...
	  vmPage.setShowInactiveModules(null);
	  ...
	  public boolean isShowInactiveModules()
	  {
		  FacesContext ctx = FacesContext.getCurrentInstance();
		  try{
			  if (showInactiveModulesMaterial == null)
			  {
				  String site_id = ToolManager.getCurrentPlacement().getContext();
				  ValueBinding binding = Util.getBinding("#{authorPreferences}");
				  AuthorPreferencePage preferencePage = (AuthorPreferencePage) binding.getValue(ctx);
				  showInactiveModules = preferencePage.isMaterialShowInactiveModules(site_id);
				  showInactiveModulesMaterial = new Boolean(showInactiveModules);
			  }
		  }
		  catch(Exception e){e.printStackTrace();
		  showInactiveModules=true;}
		  return showInactiveModules;
	  }
	...
	private boolean isUserAMemberOfAnyGroups(Set<AccessGroupService> groups){
 		if(groups != null){
 			try{
 				Collection userGroups = SiteService.getSite(ToolManager.getCurrentPlacement().getContext()).getGroupsWithMember(getUserId());
 				if (userGroups==null || userGroups.isEmpty()) {
 					return false;
 				}
 
 				for (Iterator iterator = groups.iterator(); iterator.hasNext();) {
 					AccessGroupService accessGroupService = (AccessGroupService) iterator
 					.next();
 					Iterator userGroupsIter = userGroups.iterator();
 					while (userGroupsIter.hasNext()) {
 						Group group = (Group) userGroupsIter.next();
 						if (group.getId().equals(accessGroupService.getGroupId())) {
 							return true;
 						}
 					}
 				}
