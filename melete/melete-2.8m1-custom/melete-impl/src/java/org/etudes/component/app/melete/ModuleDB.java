	 ...
	 public List getViewModules(String courseId) throws Exception {
		 Connection dbConnection = null;
			...
			try {
		    	...
		    	ResultSet rs,agRs = null;

	            String sql = "select m.module_id,c.seq_no,m.title as modTitle, m.group_restriction_flag as groupRestrictionFlag, m.subtitle as modSubtitle, m.whats_next,m.seq_xml,d.start_date,d.end_date,s.section_id,s.content_type,s.title as secTitle,s.subtitle as secSubtitle from melete_module m inner join melete_module_shdates d on m.module_id=d.module_id inner join melete_course_module c on m.module_id=c.module_id left outer join melete_section s on m.module_id = s.module_id where c.course_id = ? and c.delete_flag=0 and c.archv_flag=0 and (s.delete_flag=0 or s.delete_flag is NULL) order by c.seq_no";
...
//		    			Associate vsBeans to vmBean
		    			...
		    			if (sectionId != 0)
		    			{
		    				...
		    				vsBean.setSubtitle(rs.getString("secSubtitle"));
		    			...
		    			if ((prevModId == 0)||(moduleId != prevModId))
		    			{
		    				...
		    				vmBean.setSubtitle(rs.getString("modSubtitle"));
		    				vmBean.setWhatsNext(rs.getString("whats_next"));
		    				vmBean.setSeqXml(seqXml);
						vmBean.setGroupRestrictionFlag(rs.getBoolean("groupRestrictionFlag"));
		    				if(vmBean.getGroupRestrictionFlag()){
		    					Set<AccessGroupService> accessGroups = new HashSet();
		    					
		    					//grab all groups:
		    					sql = "select ag.group_id as groupId from melete_access_group ag where module_id = ?";
		    					PreparedStatement agPs =dbConnection.prepareStatement(sql);
		    					agPs.setString(1, "" + moduleId);
		    					agRs = agPs.executeQuery();
		    					if (agRs != null)
		    			    		{
		    				    		String groupId;
		    			    			while (agRs.next())
		    			    			{
		    			    				groupId = agRs.getString("groupId");
		    					...
		    					vmBean.setAccessGroups(accessGroups);
...
// one more attempt
    public void deleteModules(List delModules, List allModules, String courseId, String userId) throws Exception
	{
		...
		try
		{
			...
			String delAccessGroupsStr = "delete AccessGroup ag where ag.module.moduleId in " + delModuleIds;
			...
			{
				deletedEntities = session.createQuery(delAccessGroupsStr).executeUpdate();
				logger.debug("access groups deleted" + deletedEntities);
