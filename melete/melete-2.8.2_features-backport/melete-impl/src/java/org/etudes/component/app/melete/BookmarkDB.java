...
public class BookmarkDB {
		     ...
		     Query q=session.createQuery("select sa from SpecialAccess sa,CourseModule cm where sa.users like :userId and sa.moduleId=cm.moduleId and cm.courseId = :siteId");
			  ...
			  if (bmList != null)
			  {
			   	 for (ListIterator i = bmList.listIterator(); i.hasNext(); )
			 	 {
			 		 ...
			 		 {	 
			 			 boolean hasGroupAccess = true;
			 			 if(bmark.getSection().getModule().getGroupRestrictionFlag()){
			 				 hasGroupAccess = isUserAMemberOfAnyGroups(bmark.getSection().getModule().getAccessGroups(), userId);
			 			 }
			 			 if(!hasGroupAccess)
			 				 bmark.setSectionVisibleFlag(false);
			 			 else
	...
	private boolean isUserAMemberOfAnyGroups(Set<AccessGroupService> groups, String userId){
		if(groups != null){
			try{
				Collection userGroups = SiteService.getSite(ToolManager.getCurrentPlacement().getContext()).getGroupsWithMember(userId);
