/**********************************************************************************
 *
 * $URL: https://source.sakaiproject.org/contrib/etudes/melete/tags/2.8.2/melete-impl/src/java/org/etudes/component/app/melete/MeleteSecurityServiceImpl.java $
 * $Id: MeleteSecurityServiceImpl.java 69897 2010-08-24 22:44:41Z mallika@etudes.org $
 ***********************************************************************************
 *
 ...
 */
public class MeleteSecurityServiceImpl implements MeleteSecurityService,EntityProducer,EntityTransferrer,EntityTransferrerRefMigrator {
...
// setup a security advisor
   ...
   public Map<String, String> transferCopyEntitiesRefMigrator(String fromContext, String toContext, List resourceIds, boolean cleanup){
    	Map<String, String> transversalMap = new HashMap<String, String>();
    	transversalMap.putAll(transferCopyEntitiesRefMigrator(fromContext, toContext, resourceIds));
	...
	public void updateEntityReferences(String toContext, Map<String, String> transversalMap){
		if(transversalMap != null && transversalMap.size() > 0){
			...
			List<Module> moduleList = getModuleService().getModules(toContext);
			for (Iterator iterator = moduleList.iterator(); iterator.hasNext();) {
				Module module = (Module) iterator.next();
				Map sectionsMap = module.getSections();
				if(sectionsMap != null && sectionsMap.size() > 0){
					Iterator keyValuePairs = sectionsMap.entrySet().iterator();
					while (keyValuePairs.hasNext())
					{
						Map.Entry entry = (Map.Entry) keyValuePairs.next();
						Section section = (Section) entry.getValue();
						if("typeEditor".equals(section.getContentType())){
							//Search content to see if there are any references that need updating:		
							try{
								this.pushAdvisor();
								ContentResourceEdit edit = getContentservice().editResource(section.getSectionResource().getResource().getResourceId());
								String msgBody = new String(edit.getContent());
								boolean updated = false;
								Iterator<Entry<String, String>> entryItr = entrySet.iterator();
								while(entryItr.hasNext()) {
									Entry<String, String> entry2 = (Entry<String, String>) entryItr.next();
									String fromContextRef = entry2.getKey();
									if(msgBody.contains(fromContextRef)){									
										msgBody = msgBody.replace(fromContextRef, entry2.getValue());
										updated = true;
									}								
								}	
								if(updated){							
									edit.setContent(msgBody.getBytes());
									getContentservice().commitResource(edit);
								}
							}catch(PermissionException e)
							{
								logger.debug("updateEntityReferences: permission is denied", e);
							}catch (IdUnusedException e)
							{
								logger.debug("updateEntityReferences: IdUnusedException", e);
							}catch(TypeException e)
							{
								logger.debug("updateEntityReferences", e);
							}
							catch(OverQuotaException e)
							{
								logger.debug("updateEntityReferences: Adding this resource would place this account over quota.To add this resource, some resources may need to be deleted.", e);
							}catch(InUseException e)
							{
								logger.debug("updateEntityReferences: InUseException thrown", e);
							}catch(ServerOverloadException e)
							{
								logger.debug("updateEntityReferences: Server overload exception", e);
							}
						}
					}
				}
