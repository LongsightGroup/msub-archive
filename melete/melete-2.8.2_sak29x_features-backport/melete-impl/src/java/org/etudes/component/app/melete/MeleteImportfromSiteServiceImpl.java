...
public class MeleteImportfromSiteServiceImpl extends MeleteImportBaseImpl implements MeleteImportfromSiteService{
	...
	throws MalformedURLException, UnknownHostException, MeleteException, Exception {
		...
		if (section.getContentType().equals("typeEditor")) {
			...
			newResourceId = processEmbedDatafromHTML(hrefVal,sectionResourceName, courseId,addCollId, oldCourseId);
	...
	private String processEmbedDatafromHTML(String ref_id,String fileResourceName, String toSiteId,String uploadCollId) throws Exception {
		return processEmbedDatafromHTML( ref_id, fileResourceName, toSiteId, uploadCollId, null);
	}
	/*
	 *  process HTML resources.
	 */
	private String processEmbedDatafromHTML(String ref_id,String fileResourceName, String toSiteId,String uploadCollId, String oldCourseId) throws Exception
	{
		ContentResource cr1 = getMeleteCHService().getResource(ref_id);
		...
		ArrayList content = createHTMLFile(contentEditor,toSiteId,new HashSet<String>() ,parentStr, oldCourseId);
	...
	protected ArrayList createHTMLFile(String contentEditor, String courseId, Set<String> checkEmbedHTMLResources, String parentRef)throws Exception{
		return createHTMLFile ( contentEditor,  courseId, checkEmbedHTMLResources,  parentRef, null);
	}
	/*
	 ...
	 */
	protected ArrayList createHTMLFile(String contentEditor, String courseId, Set<String> checkEmbedHTMLResources, String parentRef, String oldCourseId)throws Exception{
		...
		if (Boolean.parseBoolean(ServerConfigurationService.getString("melete.useSakaiResources", "false"))) {
			contentEditor = contentEditor.replace(oldCourseId, courseId);
		}
			...
			if (sectionList != null)
			{
				...
				if (mapSize > 0)
				{
					...
					while (keyValuePairs.hasNext())
					{
						...
						Section toSec = new Section(fromSec.getTitle(), fromSec.getSubtitle(), fromSec.getCreatedByFname(), fromSec.getCreatedByLname(), fromSec.getModifiedByFname(), fromSec.getModifiedByLname(), fromSec.getInstr(), fromSec.getContentType(), fromSec.isAudioContent(), fromSec.isVideoContent(), fromSec.isTextualContent(), fromSec.isOpenWindow(), fromSec.isDeleteFlag(), fromSec.getCreationDate(), fromSec.getModificationDate());
