    ...
    public boolean isShowInactiveModules()
	  {
		  FacesContext ctx = FacesContext.getCurrentInstance();
		  try{
			if(showInactiveModules == null)
			{
		   ValueBinding binding = Util.getBinding("#{authorPreferences}");
	 	   AuthorPreferencePage preferencePage = (AuthorPreferencePage)binding.getValue(ctx);
	 	   if (courseId == null) getCourseId();
	 	  showInactiveModules = new Boolean(preferencePage.isMaterialShowInactiveModules(courseId));
			}
		  }
		  catch(Exception e){e.printStackTrace();
		  showInactiveModules=true;}
		  return showInactiveModules.booleanValue();
	  }

  public void setShowInactiveModules(Boolean showInactiveModules)
  {
  	this.showInactiveModules = showInactiveModules;
  }
}
