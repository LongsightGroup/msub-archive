...
(function()
{
    function setBase64ToImageUrl(URL, dialog, pageId, elementId) {
	debugger;
	var img = new Image();
	img.src = URL;
	img.onload = function () {
	    ...
	    dialog.setValueOf(pageId,elementId,dataURL);
	...
	var imageDialog = function( editor, dialogType )
	{
		...
		return {
			title : editor.lang.image[ dialogType == 'encodedimage' ? 'title' : 'titleButton' ],
			minWidth : 420,
			minHeight : 360,
			onShow : function()
			{
			...
			onOk : function()
			{
				...
				if ( this.imageEditMode )
				{
					...
					if ( dialogType == 'encodedimage' && imgTagName == 'input' && confirm( editor.lang.image.button2Img ) )
					{
						// Replace INPUT-> IMG
						imgTagName = 'img';
						this.imageElement = editor.document.createElement( 'img' );
					...
					else if ( dialogType != 'encodedimage' && imgTagName == 'img' && confirm( editor.lang.image.img2Button ))
					{
						// Replace IMG -> INPUT
						imgTagName = 'input';
			...
			contents : [
				{
					...
					[
						...
						},
						{
							id : 'btnEncode',
							type : 'button',
							label : editor.lang.encodedimage.encodelabel,
							title : editor.lang.encodedimage.encodetitle,
							accessKey : 'E',
							'default' : '',
							onClick : function()
							{
								dialog = this.getDialog();  
				                                url = CKEDITOR.tools.trim( dialog.getValueOf( 'info', 'txtUrl' ));
								if (!url) {
									alert (editor.lang.encodedimage.nourl);
									//TODO: Prevent state from changing
								}
								else {
								  setBase64ToImageUrl(url,dialog,'info','txtUrl');
								}
							},
							setup : function( type, element )
							{
	...
	CKEDITOR.dialog.add( 'encodedimage', function( editor )
		{
			return imageDialog( editor, 'encodedimage' );
		});

	CKEDITOR.dialog.add( 'encodedimagebutton', function( editor )
		{
			return imageDialog( editor, 'encodedimagebutton' );
		});
})();
