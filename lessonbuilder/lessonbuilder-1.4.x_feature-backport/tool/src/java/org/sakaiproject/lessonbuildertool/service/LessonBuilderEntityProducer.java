   ...
   public boolean willArchiveMerge()
   {
    ...
    private void migrateEmbeddedLinks(String toContext, Map<String, String> transversalMap){
		...
		try {
			...
			logger.warn("bad site Id in lessonbuilder link migration:"+toContext);
			return;
		}
		List<SimplePage> thisSitePages = simplePageToolDao.getSitePages(toContext);
		logger.info("LessonBuilderEntityProducer - number of pages"+String.valueOf(thisSitePages.size()));
		Iterator thisSitePagesIterator = thisSitePages.iterator();
		while(thisSitePagesIterator.hasNext()){
			SimplePage thisSimplePage = (SimplePage) thisSitePagesIterator.next();
			String thisSimplePageSitePageId = thisSimplePage.getToolId();
			SitePage thisLessonBuilderSitePage = toSite.getPage(thisSimplePageSitePageId);
			if(thisLessonBuilderSitePage!=null){
				logger.info("Found active LB Page:"+thisSimplePage.getPageId());
				updateLbPage(thisSimplePage.getPageId(), entrySet);
			}else if(thisSimplePage.getToolId().equals("0") && thisSimplePage.getTopParent()==0 && thisSimplePage.getParent()==0 && thisSimplePage.getSiteId().equals(toContext)){
				logger.info("Found a sub Page:"+thisSimplePage.getPageId());
				updateLbPage(thisSimplePage.getPageId(), entrySet);				
			}
	...
	private void updateLbPage(long simplePageId, Set entrySet){
		
		List<SimplePageItem> pageItemList = simplePageToolDao.findItemsOnPage(simplePageId);
		Iterator pageItemIterator = pageItemList.iterator();
		while(pageItemIterator.hasNext()){
			SimplePageItem thisSimplePageItem = (SimplePageItem) pageItemIterator.next();
			logger.info("LessonBuilderEntityProducer - SimplePageItem - type:"+String.valueOf(thisSimplePageItem.getType()));
			if(thisSimplePageItem.getHtml()!=null){
				logger.info("LessonBuilderEntityProducer - html:"+thisSimplePageItem.getHtml());
				String msgBody = thisSimplePageItem.getHtml();
				try {
					msgBody = editLinks(msgBody, "sam_pub");
					msgBody = editLinks(msgBody, "/posts/");
					msgBody = miagrateAllLinks(entrySet, msgBody);
					thisSimplePageItem.setHtml(msgBody);
					logger.info("html - (post mod):"+msgBody);
					simplePageToolDao.deleteItem(thisSimplePageItem);
					simplePageToolDao.quickSaveItem(thisSimplePageItem);
					logger.info("saving SimplePageItem:"+thisSimplePageItem.getId());
				} catch (Exception e) {
					// TODO Auto-generated catch block
					logger.warn("Problem migrating links in Lessonbuilder"+e.getMessage());
				}

			}
//			logger.info("LessonBuilderEntityProducer - SimplePageItem - url:"+thisSimplePageItem.getURL());					
		}		
	}
   ...
   */
                           msgBody = miagrateOneLink(fromContextRef, targetContextRef, msgBody);
                   }
                   try {
