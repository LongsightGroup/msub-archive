...
public class IrubricBean implements Serializable {
	...
	//DN 2012-11-29: issue irurbic tool: create string summary report(use set gradebookitem to irubric site) 
	...
	//Issue 53: add param rubricid from table GB_GRADABLE_OBJECT_IRUBRIC_T
	...
	public int getIrubricSwitch() {
		return Integer.parseInt(serverConfigurationService.getString("irubric.switch", "0")); 
	...
	private String getCurrentUserRole(String siteId) throws IdUnusedException, Exception {
		...
		if (roleName == null) {
			//DN 2012-12-05: fix error when admin is deleted in site
			if(userId.equals("admin")){
				//assume the "instructor" role in site
				roleName = "instructor";
			}else {
	...
	public String buildPostData(HttpServletRequest request, String purpose, String siteId)
		...
		if (purpose.equals(CMD_ATTACH)) {
			...
			if(purpose.equals(CMD_GRADE_ALL) && gradebookItemId != 0) {
				...
				studentId = gradebookManager.getStudentUIdsByGradebookItemId(gradebookItemId);
		...
		//DN 2012-11-29: Issue irubric tool: view summary report	
			...
			//DN 2012-11-29: Issue irubric tool: is get grade by gradable then set point possible
	...
	public boolean isIRubricAvailable(long gradebookItemId) {
		...
		// for the site.
		GradableObjectRubric rubric = gradebookManager.getGradableObjectRubric(gradebookItemId);
		if (rubric != null) {
			enabled = true;
		}
		return enabled;
	...
	public void saveGradeFromGB2(long gradebookItemId, String strScoreStream) {
		...
		if (length > 0) {
			...
			for(int i = 0; i< length; i ++){
				...
				//get AssignmentGradeRecord use for update grade
				...
				if(agr != null && (agr.getPointsEarned() == null || !agr.getPointsEarned().equals(score))){
						...
						//DN 2012-08-10:use for case grade never save in AssignmentGradeRecord
						//need set value use for save record AssignmentGradeRecord
						if(agr.getStudentId() == null){
							//set student id(value need save in table GB_GRADE_RECORD_T)
							agr.setStudentId(studentUId);
							//set assignment((value 'GRADABLE_OBJECT_ID' need save in table GB_GRADE_RECORD_T))
