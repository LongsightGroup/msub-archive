...
public abstract class BaseUserDirectoryService implements UserDirectoryService, StorageUser, UserFactory
{
	...
	protected void ensureMappedIdForProvidedUser(UserEdit user)
	{
		if (user.getId() == null)
		{
            if (serverConfigurationService().getBoolean("user.recheck.eid.mapping", false) ){
                String userId = m_storage.checkMapForId(user.getEid());
                if (userId != null) {
                    user.setId(userId);
                    return;
                }
	...
	public class BaseUserEdit implements UserEdit, SessionBindingListener
	{
		...
		public String getSortName()
		{
			if (m_sortName == null)
			{
				...
				//m_sortName = m_sortName.toLowerCase();
			}
		...
		public boolean equals(Object obj)
		{
			...
			if (((User) obj).getId() == null) {
				if(((User) obj).getEid() == null) {
					return false;
				}
				else
				{
					return ((User) obj).getEid().equals(getEid());
				}
			}
