...
public class Validator
{
	...
	/**
	 * Determine how a file resource should be opened.
	 * 
	 * @param contentType
	 *        The content type to check
	 * @return A string identifying the window in which to open the resource: 
	 * 		_blank    = Opens the linked document in a new window or tab
	 * 		_self     = Opens the linked document in the same frame as it was clicked (this is default)
	 * 		_parent   = Opens the linked document in the parent frame
	 * 		_top      = Opens the linked document in the full body of the window
	 * 		framename = Opens the linked document in a named frame 
	 * 		or an empty string if the resource is not a file.
	 */
	public static String getResourceTarget(String contentType)
	{
		...
		if (contentType != null)
		{
			if (letBrowserInline(contentType)) {
				// allow inline configurations
				// i.e. content.mime.inline.config.text/html=_self
				String inlineConfig = ServerConfigurationService.getString("content.mime.inline.config." + contentType);
				if (StringUtils.isNotBlank(inlineConfig)) {
					rv = inlineConfig;
				}
			} else {
