...
//      data.setSecuredIPAddressSet(securedIPAddressSet);
  ...
  {
    section.setTitle(FormattedText.unEscapeHtml((String) sectionMap.get("title"))); //ONC-3238
    section.setDescription(XmlUtil.processFormattedText(log, makeFCKAttachment((String) sectionMap.get("description"))));

    Set metaDataSet = prepareMetaDataSet(section, sectionMap);
    section.setSectionMetaDataSet(metaDataSet);   
  }
  
  private Set prepareMetaDataSet(SectionFacade section, Map sectionMap) {
	  ...
	  while(it.hasNext()) {
		  ...
		  if(pairs.getKey().equals("question_order") && !pairs.getValue().equals("")) {
			  sectionMetaData = new SectionMetaData(section.getData(), SectionMetaDataIfc.QUESTIONS_ORDERING, (String)pairs.getValue());
		  }
		  if(pairs.getKey().equals("question_order") && pairs.getValue().equals("")) {
			  sectionMetaData = new SectionMetaData(section.getData(), SectionMetaDataIfc.QUESTIONS_ORDERING, SectionDataIfc.AS_LISTED_ON_ASSESSMENT_PAGE.toString());
		  }
		  if(pairs.getKey().equals("keyword") && !pairs.getValue().equals("")) {
			  sectionMetaData = new SectionMetaData(section.getData(), SectionMetaDataIfc.KEYWORDS, (String)pairs.getValue());
		  }
		  if(pairs.getKey().equals("objective") && !pairs.getValue().equals("")) {
			  sectionMetaData = new SectionMetaData(section.getData(), SectionMetaDataIfc.OBJECTIVES, (String)pairs.getValue());
		  }
		  if(pairs.getKey().equals("rubric") && !pairs.getValue().equals("")) {
			  sectionMetaData = new SectionMetaData(section.getData(), SectionMetaDataIfc.RUBRICS, (String)pairs.getValue());
		  }
		  metaDataSet.add(sectionMetaData);
	  }
	  if(metaDataSet.contains(null)) {
		  metaDataSet.remove(null);
	  }	  
	  return metaDataSet;
  }
