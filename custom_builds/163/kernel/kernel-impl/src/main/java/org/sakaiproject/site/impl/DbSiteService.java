...
public abstract class DbSiteService extends BaseSiteService
{
	...
	protected class DbStorage extends BaseDbFlatStorage implements Storage, SqlReader
	{
		...
		private String getSitesWhere(SelectionType type, Object ofType, String criteria, Map propertyCriteria, SortType sort)
		{
			...
			if ((type == SelectionType.ACCESS) || (type == SelectionType.UPDATE) || (type == SelectionType.DELETED) || (type == SelectionType.ACCESS_ALL) || (type == SelectionType.ACTIVE) || (type == SelectionType.INACTIVE))
			{
			...
			if (type == SelectionType.INACTIVE) {
		...
		private Object[] getSitesFields(SelectionType type, Object ofType, String criteria, Map propertyCriteria)
		{
			...
			if ((type == SelectionType.JOINABLE) || (type == SelectionType.ACCESS) || (type == SelectionType.UPDATE) || (type == SelectionType.ACCESS_ALL) || (type == SelectionType.ACTIVE) || (type == SelectionType.INACTIVE)) fieldCount++;
		...
		public int countSites(SelectionType type, Object ofType, String criteria, Map propertyCriteria)
		{
			...
			if (criteria != null) fieldCount += 1;
			if ((type == SelectionType.JOINABLE) || (type == SelectionType.ACCESS) || (type == SelectionType.UPDATE) || (type == SelectionType.DELETED) || (type == SelectionType.ACCESS_ALL) || (type == SelectionType.ACTIVE) || (type == SelectionType.INACTIVE)) fieldCount++;
			if (propertyCriteria != null) fieldCount += (2 * propertyCriteria.size());
			Object fields[] = null;
			if (fieldCount > 0)
			{
      ...
      public List<ToolConfiguration> findTools(final String toolReg) {
			String sql = siteServiceSql.getToolsOfTypeFieldsSql();
			...
			List found = m_sql.dbRead(sql, fields, new SqlReader()
			{
				public Object readSqlResultRecord(ResultSet result)
				{
					try
					{
						...
						// make the tool
						BaseToolConfiguration tool = new BaseToolConfiguration(DbSiteService.this,id, toolReg, title, layout, pageId, siteId, skin, pageOrder);

						return tool;
					}
					catch (SQLException e)
					{
						M_log.warn("findTool: " + toolReg + " : " + e);
						return null;
					}
				}
			});
