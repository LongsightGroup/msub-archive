...
public abstract class BaseSiteService implements SiteService, StorageUser
{
	...
	public Site addSite(String id, Site other) throws IdInvalidException, IdUsedException, PermissionException
	{
		...
		try
		{
            ...
            // fix for UCDAVIS-204
            site.addMember(userDirectoryService().getCurrentUser().getId(), re.getMaintainRole(), true, false);

            if(re.getProviderGroupId() != null){
                re.setProviderGroupId("");
            }
   ...
   public List<ToolConfiguration> findTools(final String toolReg) {
      return m_storage.findTools(toolReg);
   }
