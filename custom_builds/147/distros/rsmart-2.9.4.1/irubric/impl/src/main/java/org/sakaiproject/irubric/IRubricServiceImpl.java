...
public class IRubricServiceImpl implements Serializable, IRubricService {
     ...
     * @param sGrade
     */
    public String processOneGrade(String sGrade) {
        if (sGrade != null) {
            switch (sGrade.charAt(0)) {
                case 'R':
                    ...
                    return newScore;
                //TODO handle conversion to letter grade type


/*                    if (gradebook != null) {
                        int gradeEntry = gradebook.getGrade_type();
                        if (gradeEntry == GradebookService.GRADE_TYPE_LETTER) {
                            LetterGradePercentMapping lgpm = iRubricBean.getGradebookService().getFixedGrade(gradebook.getUid());
                                    .getLetterGradePercentMapping(gradebook);
                            Double pointsPossible = iRubricBean.getPointsPossible();
                            newScore = iRubricBean.getGradebookService()
                    ...
                    //break;
                case 'N':
                    LOG.info("iRubric has not graded this item.");
                    break;
                case 'E':
                    LOG.info("iRubric does not have a rubric attached for this gradebook item.");
                    break;
                default:
    ...
    public String buildPostDataForGradeAll(String gradebookUid, Long gradebookItemId, String purpose)
        ...
        if (purpose.equals(CMD_GRADE_ALL) && gradebookItemId != 0) {
            ...
            studentId = getiRubricManager().getStudentUIdsByGradebookItemId(gradebookItemId);
