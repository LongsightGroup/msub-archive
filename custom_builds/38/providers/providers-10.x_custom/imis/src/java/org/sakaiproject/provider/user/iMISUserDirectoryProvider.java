...
import com.isgsolutions.ibridge.authentication.AuthenticationX0020WebX0020Service;
import com.isgsolutions.ibridge.authentication.AuthenticationX0020WebX0020ServiceSoap;
import com.isgsolutions.ibridge.dataaccess.DataAccessX0020WebX0020Service;
import com.isgsolutions.ibridge.dataaccess.DataAccessX0020WebX0020ServiceSoap;
 ...
 * </p>
 */
public class iMISUserDirectoryProvider implements UserDirectoryProvider,
		UsersShareEmailUDP, DisplayAdvisorUDP, ExternalUserSearchUDP {
	/** Our log (commons). */
	private static Log M_log = LogFactory
			.getLog(iMISUserDirectoryProvider.class);
	...
	/*
	 * protected Hashtable m_info = null; protected Call authUser = null;
	 * protected Call dataAccess = null;
	 * 
	 * protected class Info { public String id;
	 * 
	...
	protected boolean userExists(String userId) {
		...
		DataAccessX0020WebX0020ServiceSoap dataPort = dataService
				.getDataAccessX0020WebX0020ServiceSoap();

		ExecuteDatasetStoredProcedureResult result = dataPort
				.executeDatasetStoredProcedure(spStoredProcedure,
						"iweb_sp_getUsersByid_SAKAI", userId);
	...
	public boolean getUser(UserEdit edit) {
		...
		ExecuteDatasetStoredProcedureResult result = dataPort
				.executeDatasetStoredProcedure(spStoredProcedure,
						"iweb_sp_getUsersByid_SAKAI", edit.getEid());
		...
		if (result == null) {
			...
			try {
				...
				if (firstStr.equals("")) {
					M_log.warn("I can't find First Name in the xml for "
							+ edit.getEid());
				}
				if (lastStr.equals("")) {
					M_log.warn("I can't find Last Name in the xml for "
							+ edit.getEid());
				}
				if (emailStr.equals("")) {
					M_log.warn("I can't find email in the xml for "
							+ edit.getEid());
				}
				...
				edit.setPassword(""); // TODO - does this have to be set
										// correctly? Does it matter since
										// authenticateUser will be consulted
										// anyway?
	 ...
	 * This is the sort of method to provide if your external source has only a
	 * single user for any email address.
	 * 
	 * @param email
	 ...
	 *            The email address string.
	 * @param factory
	 *            Use this factory's newUser() method to create all the UserEdit
	 *            objects you populate and return in the return collection.
	 ...
	 *            The user id.
	 * @param edit
	 *            The UserEdit matching the id to be authenticated (and updated)
	 *            if we have one.
	 * @param password
	 *            The password.
	 * @return true if authenticated, false if not.
	 */
	public boolean authenticateUser(String userId, UserEdit edit,
			String password) {
		...
		AuthenticationX0020WebX0020ServiceSoap authPort = authService
				.getAuthenticationX0020WebX0020ServiceSoap();
		String authResult = authPort.authenticateUser(spAuthenticateUser,
				userId, password);

		try {
			InputStream res = new ByteArrayInputStream(
					authResult.getBytes("UTF-16"));
	...
	public List<UserEdit> searchExternalUsers(String criteria, int first,
			int last, UserFactory factory) {
		...
		ExecuteDatasetStoredProcedureResult result = dataPort
				.executeDatasetStoredProcedure(spStoredProcedure,
						"iweb_sp_getUsersByid_SAKAI", criteria);
		...
		if (result != null) {
			try {
				...
				user.setPassword("ffdsfsgsjhdfvdsfvhdsvc"); // TODO - does this
															// have to be set
															// correctly?
