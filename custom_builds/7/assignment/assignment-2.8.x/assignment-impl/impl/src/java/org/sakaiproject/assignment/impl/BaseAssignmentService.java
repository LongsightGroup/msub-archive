...
//	spring service injection
	...
	protected void zipSubmissions(String assignmentReference, String assignmentTitle, String gradeTypeString, int typeOfSubmission, Iterator submissions, OutputStream outputStream, StringBuilder exceptionMessage, boolean withStudentSubmissionText, boolean withStudentSubmissionAttachment, boolean withGradeFile, boolean withFeedbackText, boolean withFeedbackComment, boolean withFeedbackAttachment) 
	{
		...
		try {
			...
			gradesBuffer.append(rb.getString("grades.id") + "\t" + rb.getString("grades.eid") + "\t" + rb.getString("grades.lastname") + "\t" + rb.getString("grades.firstname") + "\t" + rb.getString("grades.grade") + "\t" + rb.getString("grades.comment") + "\n");
					...
					try
					{
						...
						if (allowAddSubmissionUsers.contains(u))
						{
							...
							for (int i = 0; i < submitters.length; i++)
							{
								...
								gradesBuffer.append(submitters[i].getDisplayId() + "\t" + submitters[i].getEid() + "\t" + submitters[i].getLastName() + "\t" + submitters[i].getFirstName() + "\t");
 								if(withGradeFile){
 									gradesBuffer.append(s.getGradeDisplay());
 								}
 								gradesBuffer.append("\t");
 								if(withFeedbackComment){
 									gradesBuffer.append(s.getFeedbackComment().replaceAll("\t", "        "));
 								}
 								gradesBuffer.append("\n");
							}
...
//								}
								...
								if (typeOfSubmission != Assignment.TEXT_ONLY_ASSIGNMENT_SUBMISSION && typeOfSubmission != Assignment.NON_ELECTRONIC_ASSIGNMENT_SUBMISSION)
								{
									...
									if (withStudentSubmissionAttachment)
									{
										...
										String sSubAttachmentFolder = submittersName;// + rb.getString("stuviewsubm.submissatt") + "_";
...
//								}
								...
								if (withFeedbackAttachment)
								{
									...
									String feedbackSubAttachmentFolder = submittersName;// + rb.getString("download.feedback.attachment") + "_";
			...
			{
				// continue
				if (withGradeFile || withFeedbackComment)
				{
