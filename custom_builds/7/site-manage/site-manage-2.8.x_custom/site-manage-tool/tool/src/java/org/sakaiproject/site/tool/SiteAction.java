...
public class SiteAction extends PagedResourceActionII {
	...
	private String buildContextForTemplate(String preIndex, int index, VelocityPortlet portlet,
		...
		case 0:
			...
			List<AcademicSession> aSessions = setTermListForContext(context, state, true);
			if(aSessions != null){
		...
		case 27:
			...
			// added by bjones86 - back-porting SAK-21323 from Sakai 2.9.x for LB import-merge support,
			//		using UVa's back-port for 2.7.1 as reference:
			//		The 2.7.1 version of orderToolIds() does not include the final boolean method argument;
			//		the Sakai 2.8.x tag adds this, but does not reference it in the method;
			//		so, remove the final boolean arg from the 2.9.x patch for 2.7.1 use;
			//		however, since we are starting from 2.8.x, we can still include this final boolean argument.
			context.put("selectedTools", orderToolIds(state, site_type,
			        originalToolIds( (List<String>) importableTools, state ), false)); // String toolId's
			context.put("importSites", state.getAttribute(STATE_IMPORT_SITES));
	...
	*/
	...
	// added by bjones86 - Chuck Hedrick (SAK-21323):
	// Replace fake tool ids with real ones. Don't duplicate, since several fake tool ids may appear for the same real one
	// it's far from clean that we need to be using fake tool ids at all. But I don't know the code well enough to get rid
	// of the fakes completely
			...
			if (forward) {
				...
				if (existingSite != null) {
					...
					if (select_import_tools(params, state)) {
						...
						// Added by bjones86 backport of SAK-21323
						List selectedTools = originalToolIds( (List<String>) state
	...
	private void getCourseOfferingAndSectionMap(String academicSessionEid, HashMap courseOfferingHash, HashMap sectionHash, String sectionEid, Section section) {
		try {
			...
			M_log.warn(this + ".getCourseOfferingAndSectionMap:" + " cannot find section id=" + sectionEid);
		}
		if (section != null) {
