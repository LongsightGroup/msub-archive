...
public class ITunesUAdminHandler {
    ...
    public String processLookupCourse() {
    	...
    	else
    	{
    		...
    		lookupHandle = itunesuService.getITunesUCourseHandle(lookupSiteId, "", prefix, destination, token);
    	}
    ...
    public String processLookupOrphanedPage() 
	{
		...
		orphanedPages = itunesuService.getOrphanedPages(prefix, destination, token);
        ToolSession session = sessionManager.getCurrentToolSession();
        session.setAttribute(ATTR_TOP_REFRESH, Boolean.TRUE);
	...
	{	
		...
		try
		{
			...
			String courseHandle = itunesuService.getITunesUCourseHandle(addFeedGroup_siteId, siteName, prefix, destination, token);
			
			// delete from iTunesU server
			String uploadURL = itunesuService.getUploadURL(courseHandle, prefix, destination, token);
			...
			itunesuService.wsCall(operation, uploadURL, xmlDocument, prefix, destination, token);
