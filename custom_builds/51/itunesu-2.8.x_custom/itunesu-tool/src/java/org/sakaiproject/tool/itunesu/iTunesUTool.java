...
public class iTunesUTool extends HttpServlet {
   ...
   /* private void generateShowTreeXml(String filename) 
     ...
     */
    void addITunesUCourse(String uploadURL)
    {
    		...
    		String rvInputString = itunesuService.wsCall(itunesuService.WS_ADD_COURSE, uploadURL, addCourseXmlDocument);
	...
	private String handleITunesU()
   	{
		...
		{	
    		...
    		if ((courseHandle = itunesuService.getITunesUCourseHandle(siteId, siteName)) == null)
    		{
    			...
    			String uploadURL = itunesuService.getUploadURL(sectionHandle);

    			// actual creation of course
    			if (uploadURL != null)
    			{
    				addITunesUCourse(uploadURL);
    			}

    			// now that's its created, let's try to get handle again
        		courseHandle = itunesuService.getITunesUCourseHandle(siteId, siteName);
        		...
        		if(itunesuService.isCustomItunesPermissionsPerSite())
        		{
            		if (courseHandle != null)
            		{
            			...
            			uploadURL = itunesuService.getUploadURL(courseHandle);
                		
                		// send the upload http request
                		itunesuService.wsCall(itunesuService.WS_ADD_PERMISSION, uploadURL, addPermissionXmlDocument);
    		...
    		String destination= t.get("destination")!=null?(String) t.get("destination") + "." + courseHandle:"";
