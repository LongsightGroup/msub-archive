...
public class SiteAction extends PagedResourceActionII {
    ...
    private SiteInfoComposer siteInfoComposer = (SiteInfoComposer) ComponentManager.get("org.sakaiproject.sitemanage.api.SiteInfoComposer");
    ...
    private static final String TOOL_ID_SYNOPTIC_JFORUM = "sakai.synoptic.jforum.tool";
	...
	static
	{
        ...
        SYNOPTIC_TOOL_ID_MAP.put(TOOL_ID_SYNOPTIC_JFORUM, new ArrayList(Arrays.asList("sakai.jforum.tool")));
	}
    ...
    public Boolean allowRequestsFromNonIOR=false;
	...
	private String buildContextForTemplate(String preIndex, int index, VelocityPortlet portlet,
		...
		case 10:
			...
			if (siteType != null && siteType.equalsIgnoreCase(courseSiteType)) {
                ...
                // fix for CLE-10544, came from SAK-21082, which we are undoing
	...
	//			}
			...
			if (siteType != null && siteType.equalsIgnoreCase(courseSiteType)) {
				...
				if (state.getAttribute(STATE_MANUAL_ADD_COURSE_NUMBER) != null) {
					...
					if (courseManagementIsImplemented()) {
                        ...
                        if(siteInfoComposer != null){
                            List<Section> fullSectionList = new ArrayList<Section>();
                            addSections(cmRequestedList, fullSectionList);
                            fullSectionList.addAll(getSectionsFromIds((List<String>)state.getAttribute(SITE_PROVIDER_COURSE_LIST)));
                            addSections(cmAuthorizerSectionList, fullSectionList);
                            //Don't overwrite the title if one already exists unless the title is not editable
                            if(siteInfo.title == null || siteInfo.title.isEmpty() ||
                                    (!Boolean.valueOf(siteTitleEditable(state, siteType)) && fullSectionList.size() > 0)){
                                siteInfo.title = siteInfoComposer.composeTitle(fullSectionList);
                            }
                            if(siteInfo.description == null || siteInfo.description.isEmpty()){
                                siteInfo.description = siteInfoComposer.composeDescription(fullSectionList);
                            }
                        }
					} else {
		...
		case 36:
            ...
            context.put("allowOfficialDescription", ServerConfigurationService.getBoolean(SiteConstants.SAKAI_PROPERTY_ALLOW_OFFICIAL_DESCRIPTION, true));
            context.put("showSectionMetadata", ServerConfigurationService.getBoolean(SiteConstants.SAKAI_PROPERTY_SHOW_SECTION_METADATA, true));
            ...
            String userId = state.getAttribute(STATE_INSTRUCTOR_SELECTED) != null ? (String) state.getAttribute(STATE_INSTRUCTOR_SELECTED) : UserDirectoryService.getCurrentUser().getEid();
            if (ServerConfigurationService.getBoolean("sitesetup.useDisplayId", false)) {
                try {
                    userId = UserDirectoryService.getUserByEid(userId).getDisplayId();
                    context.put("currentUserId", UserDirectoryService.getUserByEid((String) state.getAttribute(STATE_CM_CURRENT_USERID)).getDisplayId());
                } catch (UserNotDefinedException e) {
                    M_log.warn("UserNotDefinedException: For EID: " + userId);
                }
            }
            else {
    ...
    private List<Section> getSectionsFromIds(List<String> sectionList) {
        List sections = new ArrayList();
        if (courseManagementIsImplemented() && sectionList != null && !sectionList.isEmpty()) {
            for (String sectionId : sectionList){
                 try {
                    Section section = cms.getSection(sectionId);
                    sections.add(section);
                } catch (IdNotFoundException ide) {
                    M_log.error("can't find section with eid:" + sectionId);
                }
            }
        }
        return sections;
    ...
    private void addSections(List<SectionObject> sectionObjectList, List<Section> fullSectionList) {
        if (sectionObjectList != null) {
            for (SectionObject sectionObject: sectionObjectList) {
                fullSectionList.add(sectionObject.getSection());
			...
			if (SecurityService.isSuperUser()) {
				...
				if (view != null) {
					...
					if (ServerConfigurationService.getBoolean("sitesetup.show.unpublished", false)) {
						selectionType = org.sakaiproject.site.api.SiteService.SelectionType.ACCESS_ALL;

						if (ServerConfigurationService.getStrings("sitesetup.showUnpublishedSiteTypes") != null) {
							ofType = ServerConfigurationService.getStrings("sitesetup.showUnpublishedSiteTypes");
						}
					} else {
					...
					if (view.equals(SiteConstants.SITE_TYPE_ALL)) {
                        ...
                        size += SiteService.countSites(
                                org.sakaiproject.site.api.SiteService.SelectionType.ACTIVE,
                                view, search, termProp);
                    } else if (view.equals(SiteConstants.SITE_INACTIVE)) {
                        view = null;
                        size += SiteService.countSites(
                                        org.sakaiproject.site.api.SiteService.SelectionType.INACTIVE,
                                        ofType, search, termProp);
                    } else {
						// search for specific type of sites

						if (ofType == null || (ofType != null && StringUtils.join((Object[]) ofType).contains(view))){
					...
					if (view.equals(SiteConstants.SITE_TYPE_ALL)) {
                        ...
                        rv
                                .addAll(SiteService
                                        .getSites(
                                                selectionType,
                                                ofType, search, termProp, sortType,
                                                new PagingPosition(first, last)));
					}
					else if (view.equals(SiteConstants.SITE_TYPE_MYWORKSPACE)) {
						// get the current user MyWorkspace site
                        ...
                        rv.addAll(SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.ACTIVE,
                                null, search, termProp, sortType,
                                new PagingPosition(first, last)));
                    } else if (view.equals(SiteConstants.SITE_INACTIVE)) {
                        rv.addAll(SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.INACTIVE,
                                ofType, search, termProp, sortType,
                                new PagingPosition(first, last)));
                    } else {
						if (ofType == null || (ofType != null && StringUtils.join((Object[]) ofType).contains(view))){
	...
	public void doManual_add_course(RunData data) {
		...
		if (option.equalsIgnoreCase("change") || option.equalsIgnoreCase("add")) {
            ...
            if (checkForTitleDuplication(siteInfo.getTitle(), getStateSite(state))) {
                addAlert(state, rb.getString("java.sitewithtitle") + " " + siteInfo.getTitle()
	...
	private void readCourseSectionInfo(SessionState state,
        ...
        for(int i = 0; i < multiCourseInputs.size(); i++){

            List sectionFieldList = (List) multiCourseInputs.get(i);

            if(sectionFieldList != null && !sectionFieldList.isEmpty()){

                //Check the value of the first field, if it is null we do not need to find the section eid
                SectionField field = (SectionField)sectionFieldList.get(i);
                if (field != null && field.getValue() != null) {
                    sectionEids.add(sectionFieldProvider.getSectionEid(t.getEid(), (List) multiCourseInputs.get(i)));
                }
            }
        }
        ...
        // default title
        String title;
        if (siteInfoComposer != null && courseManagementIsImplemented()) {
            title = siteInfoComposer.composeTitle(t.getEid(), multiCourseInputs);
        } else {
        ...
        if (sectionEids != null && !sectionEids.isEmpty()) {
            ...
            siteInfo.description = descriptionBuffer.toString();
        }
    ...
    protected void filterSectionsByIOR(List<String> sectionEids) {
        if (siteInfoComposer != null && courseManagementIsImplemented()) {
            User currentUser = UserDirectoryService.getCurrentUser();
            List<String> sectionEidsList = new ArrayList(sectionEids);
            for (String sectionEid : sectionEidsList) {
                Section section = null;
                try {
                    section = cms.getSection(sectionEid);
                } catch (IdNotFoundException e) {
                    M_log.debug("not adding section:" + sectionEid + " for user:" + currentUser.getEid() + " we can't find this section");
                    continue;
                }
                if (section != null && section.getEnrollmentSet() != null) {
                    Set<String> iors = cms.getInstructorsOfRecordIds(section.getEnrollmentSet().getEid());
                    if (!iors.isEmpty()) {
                        for (String ior : iors) {
                            if (currentUser.getEid().equals(ior)) {
                                M_log.debug("adding section:" + section.getEid() + " for ior:" + ior);
                            } else {
                                M_log.debug("not adding section:" + section.getEid() + " for user:" + ior + " since they are not the ior");
                                sectionEids.remove(sectionEid);
                            }
                        }
                    } else {
                        M_log.debug("not adding section:" + section.getEid() + " instructor(s) of record not found");
                        sectionEids.remove(sectionEid);
                    }
                } else {
                    M_log.debug("not adding section:" + section.getEid() + " for user:" + currentUser.getEid() + " we can't find a section or enrollment");
		...
		if (state.getAttribute(STATE_MESSAGE) == null) {
			...
			else
			{
				...
				if (state.getAttribute(STATE_TEMPLATE_SITE_COPY_CONTENT) != null)
				{
					...
					importToolContent(templateSite.getId(), site, true, true);
					try {
					    site = SiteService.getSite(site.getId());
					} catch (Exception ee) {
            ...
            fixSections(site.getId());
		}

	}// doFinish
	
    private void fixSections(String siteId){
        Site site = null;
        try {
            site = SiteService.getSite(siteId);
            SiteService.save(site);
        } catch (Exception e) {
            throw new RuntimeException("site with id [" +siteId + "] is not found", e);
        }
    }    
	...
	*/
	...
	private void updateCourseSiteSections(SessionState state, String siteId, ResourcePropertiesEdit rp, AcademicSession term) {
        ...
        if (manualAddNumber != 0) {
            ...
            // the required fields)
            List<String> manualSectionsList = new ArrayList<String>();
            for (int j = 0; j < manualAddNumber; j++) {
                manualSectionsList.add(sectionFieldProvider.getSectionEid(term.getEid(),(List) manualCourseInputs.get(j)));
            }

            if (!allowRequestsFromNonIOR)
                filterSectionsByIOR(manualSectionsList);

            if(!manualSectionsList.isEmpty()){
                try {
                    AuthzGroup realmEdit = AuthzGroupService.getAuthzGroup(realm);
                    String providerRealm = buildExternalRealm(siteId, state, manualSectionsList, StringUtils.trimToNull(realmEdit.getProviderGroupId()));
                    realmEdit.setProviderGroupId(providerRealm);
                    AuthzGroupService.save(realmEdit);
                } catch (GroupNotDefinedException e) {
                    M_log.warn(this + ".updateCourseSiteSections: IdUnusedException, not found, or not an AuthzGroup object", e);
        ...
        if (cmRequestedSections != null
            ...
            List<String> cmRequestedSectionList = new ArrayList<String>();

            if(cmRequestedSections == null && state.getAttribute(STATE_CM_SELECTED_SECTIONS) != null){
                cmRequestedSections = (List) state.getAttribute(STATE_CM_SELECTED_SECTIONS);
            }

            for (int j = 0; j < cmRequestedSections.size(); j++) {
                cmRequestedSectionList.add(cmRequestedSections.get(j).getEid());
            }

            if (!allowRequestsFromNonIOR)
                filterSectionsByIOR(cmRequestedSectionList);

            if (!cmRequestedSectionList.isEmpty()) {
                try {
                    AuthzGroup realmEdit = AuthzGroupService.getAuthzGroup(realm);
                    String providerRealm = buildExternalRealm(siteId, state, cmRequestedSectionList, StringUtils.trimToNull(realmEdit.getProviderGroupId()));
                    realmEdit.setProviderGroupId(providerRealm);
                    AuthzGroupService.save(realmEdit);
                } catch (GroupNotDefinedException e) {
                    M_log.warn(this + ".updateCourseSiteSections: IdUnusedException, not found, or not an AuthzGroup object", e);
                ...
                cmRequestedSections.clear();
                for (Iterator it = cmRequestedSectionList.iterator(); it.hasNext(); ) {
                    cmRequestedSections.add(new SectionObject(cms.getSection(it.next().toString())));
                }
	...
	private void init(VelocityPortlet portlet, RunData data, SessionState state) {
		...
		setupFormNamesAndConstants(state);
        allowRequestsFromNonIOR = ServerConfigurationService.getBoolean("allowRequestsFromNonIOR", false);
	...
	private void actionForTemplate(String direction, int index,
					...
					if (StringUtils.trimToNull(params.getString("title")) == null) {
						...
						try {
							...
							try {
                                ...
                                // import tool content
								importToolContent(oSiteId, site, false, false);
		...
		case 44:
			if (forward) {
				...
				{
                    SiteService.save(site);
                    if ((siteInfo.site_type == null  || siteInfo.site_type.isEmpty()) && state.getAttribute(STATE_SITE_INFO) != null){
                       siteInfo = (SiteInfo) state.getAttribute(STATE_SITE_INFO);
                    }
                    if (courseManagementIsImplemented() && siteInfoComposer != null && !siteTitleEditable(state, siteInfo.site_type)) {
                       site.setTitle(siteInfoComposer.composeTitle(site));
                        SiteService.save(site);
                    }
				}
				catch (Exception e)
	 ...
	 * @param site
	 */
	private void importToolContent(String oSiteId, Site site, boolean bypassSecurity, boolean usingTemplate) {
			...
			for (ListIterator i = pageList
				...
				if (!(pageToolList == null || pageToolList.size() == 0))
				{
					...
					if (toolId.equalsIgnoreCase("sakai.resources")) {
                        ...
                        Site oldSite = null;
						try {
                            oldSite = SiteService.getSite(oSiteId);
						} catch (IdUnusedException e) {
							M_log.error("Unable to find site with ID: " + oSiteId, e);
						}
                        if (site.getDescription() == null || "".equals(site.getDescription())) {
                            if (oldSite != null) {
                                site.setDescription(oldSite.getDescription());
                            }
                        }
						commitSite(site);
					}
                    //need to make sure melete is copied after resources
                    //prob better way to do this... cle-4370
					else if (toolId.equalsIgnoreCase("sakai.melete")){
                        meleteFound = true;
                    } else {
						// other
						// tools
        ...
        if (meleteFound) {
            Map<String, String> entityMap = transferCopyEntities("sakai.melete",
                    oSiteId, nSiteId);
            if (entityMap != null) {
                transversalMap.putAll(entityMap);
            }
	...
	private void updateSiteInfo(ParameterParser params, SessionState state) {
        ...
        { 	 
            ...
            else if (checkForTitleDuplication(title, getStateSite(state)))
            {
                addAlert(state, rb.getString("java.sitewithtitle") + " " + title
                                + " " + rb.getString("java.exists"));                                
            }
	...
	private void addSynopticTool(SitePage page, String toolId,
        ...
        if (page.getContainingSite() != null) {
            if (page.getLayout() != SitePage.LAYOUT_SINGLE_COL || !page.getContainingSite().isCustomPageOrdered()) {
		...
		if (hasHome) {
			...
			for (String homeToolId: homeToolIds)
			{
				...
				else
				{
                    ...
                    if (homeInWSetupPageList == false) {
                        // then add those synoptic tools that were not there before
		...
		for (ListIterator j = orderToolIds(state, siteType, chosenList, false)
			...
			if (inWSetupPageList) {
				...
				if (toolRegFound != null) {
                    ...
                    if (homeInWSetupPageList == true) {
                        String toolName = toolRegFound.getId();
                        int synopticToolIndex = 0;
                        List<String> tempToolList = new ArrayList<String>();
                        tempToolList.add(toolName);
                        String synopticId = "";
                        Set tempSet = SYNOPTIC_TOOL_ID_MAP.entrySet();

                        for (Iterator iterator = tempSet.iterator(); iterator.hasNext(); ) {
                            Map.Entry tempEntry = (Map.Entry) iterator.next();
                            List synopticList = (ArrayList<String>) tempEntry.getValue();
                            for (Iterator it = synopticList.iterator(); it.hasNext(); ) {
                                String synopticItem = it.next().toString();
                                if (tempToolList.get(0) != null && tempToolList.get(0).equals(synopticItem)) {
                                    synopticId = tempEntry.getKey().toString();
                                    try {
                                        SitePage page = site.getPage(homePageId);
                                        // use value from map to find an internationalized tool title
                                        String toolTitleText = rb.getString(SYNOPTIC_TOOL_TITLE_MAP.get(synopticId));
                                        if(!pageHasToolId(page.getTools(), synopticId)){
                                            addSynopticTool(page, synopticId, toolTitleText, synopticToolIndex++ + ",1");
                                        }
                                    } catch (Exception e) {
                                        M_log.warn(this + ".saveFeatures addSynotpicTool: " + e.getMessage() + " site id = " + site.getId() + " tool = " + SYNOPTIC_TOOL_ID_MAP.get(synopticId).get(0), e);
                                    }
                                }
                            }
                        }
    ...
    private boolean checkForTitleDuplication(String title, Site site) {
            String siteId = null;
            if (site != null){
                siteId = site.getId();       
            }

            // UCDAVIS-47 :  new logic to ensure title uniqueness
            if (ServerConfigurationService.getBoolean("site.ensure.title.uniqueness", false)) {
                         
                List<Site> mySites = SiteService.getSites(org.sakaiproject.site.api.SiteService.SelectionType.ANY, null, title, null, null, null);

                boolean siteExists = false;
                for (Site existingSite : mySites) {
                       if (title.equals(existingSite.getTitle()) &&
                               !existingSite.getId().equals(siteId)) {
                               siteExists = true;
                               break;
                       }
                }
		...
		if (state.getAttribute(STATE_MESSAGE) == null) {
			try {
				...
				if (templateSite != null) {
                    ...
                    if(templateSite.getSkin() != null && !"".equals(templateSite.getSkin())){
                        site.setSkin(templateSite.getSkin());
                    }
				} else {
				...
				site.setJoinerRole(siteInfo.joinerRole);
                site.setPublished(siteInfo.published);
				//CLE-11188 set current user info as contact name and email
                if (siteInfo.site_contact_name == null || "".equals(siteInfo.site_contact_name)) {
                    siteInfo.site_contact_name = UserDirectoryService.getCurrentUser().getDisplayName();
                }
                if (siteInfo.site_contact_email == null || "".equals(siteInfo.site_contact_email)) {
			...
			for (int i = 0; i < selectedTools.size(); i++) 
			{
				...
				else if (isMultipleInstancesAllowed(findOriginalToolId(state, id)) && (idSelected != null && !idSelected.contains(id) || idSelected == null))
				{
					...
					if (title != null) 
					{
						...
						if (title.length() > MAX_TOOL_TITLE_LENGTH && !id.equals(WEB_CONTENT_TOOL_ID))
						{
	...
	public class SectionObject {
		...
		public String getAuthorizerString() {
			...
			if (authorizer != null && !authorizer.isEmpty())
			{
				...
				{
                    String displayId = authorizer.get(count);
                    try {
                        User authorizingUser = UserDirectoryService.getUserByEid(displayId);
                        if (authorizingUser != null) {
                            displayId = authorizingUser.getDisplayId();
                        }
                    } catch (Exception e) {
                        M_log.warn("can't find user with id:" + displayId + " using eid from cm data");
                    }
	...
	private void collectNewSiteInfo(SessionState state,
		...
		if (state.getAttribute(STATE_MESSAGE) == null) {
            ...
            if (state.getAttribute(STATE_SITE_INSTANCE_ID) != null ){
                Site site = getStateSite(state);
                if (site != null && site.getTitle() != null && site.getTitle().length() > 0) {
                    noTitle = false;
                }
            }
			...
			if (providerChosenList != null && providerChosenList.size() >= 1) {
                if (noTitle || (!Boolean.valueOf(siteTitleEditable(state, siteType)) && providerChosenList.size() > 0)) {
            ...
            if (siteInfoComposer != null && courseManagementIsImplemented()) {
                // need to construct the list of Section objects
                List<Section> sectionList = new ArrayList();
                if (providerChosenList != null) {
                    for (Iterator i=providerChosenList.iterator(); i.hasNext() ; ) {
                        String providerSectionId = (String)i.next();
                        try {
                            Section s = cms.getSection(providerSectionId);
                            if (s != null) {
                                sectionList.add(s);
                            ...
                            M_log.warn("cannot find section " + providerSectionId, e);
                        }
                    }
                }

                siteInfo.description = siteInfoComposer.composeDescription(sectionList);
		...
		if (getStateSite(state) == null)
		{
			...
			if (siteInfo.title == null || siteInfo.title.trim().length() == 0) {
				if (requestedSections.size() >= 1) {
                    ...
                    if (courseManagementIsImplemented() && siteInfoComposer != null) {
                        List sections = new ArrayList();
                        for (SectionObject so : requestedSections) {
                            sections.add(so.getEid());
                        }
                        siteInfo.title = siteInfoComposer.composeTitle(sections);
                    }
    ...
    private String prepareTitle(List sectionList, List chosenList) {
           ...
           if (courseManagementIsImplemented()) {
               ...
               for(Iterator itr = chosenList.iterator(); itr.hasNext();){
                   sections.add((Section) map.get(itr.next()));
               }
               title = siteInfoComposer.composeTitle(sections);
           } else {
...
// doInactivate_all added per CLE-4644	
	public void doInactivate_all(RunData data) {
            processActivateInactivate(data, false);
	}

// doActivate_all added per CLE-4644	
        public void doActivate_all(RunData data){
            processActivateInactivate(data, true);
        }// doActivate_all

// processActivateInactivate added per CLE-4644
        public void processActivateInactivate(RunData data, boolean status) {
		SessionState state = ((JetspeedRunData) data)
				.getPortletSessionState(((JetspeedRunData) data).getJs_peid());
		ParameterParser params = data.getParameters();
		Site s = getStateSite(state);
		...
		if (AuthzGroupService.allowUpdate(realmId)
			...
			try {
				...
				// list of roles being added or removed
				HashSet<String>roles = new HashSet<String>();
                String activateRole = params.getString("role");
				// remove all roles and then add back those that were checked
				for (int i = 0; i < participants.size(); i++) {
					String id = null;
					...
					Participant participant = (Participant) participants.get(i);
					id = participant.getUniqname();
                    String partRole = participant.getRole();
                    boolean partActive = participant.isActive();
                    boolean fromProvider = !participant.isRemoveable();
					if (partRole.equals(activateRole)){
                        partActive = status;
                    }
					realmEdit.addMember(id, partRole, partActive, fromProvider);
