...
public class DbAssignmentService extends BaseAssignmentService
{
	...
	protected class DbCachedAssignmentSubmissionStorage extends BaseDbSingleStorage implements AssignmentSubmissionStorage
	{
		...
		public AssignmentSubmission get (String assignmentId, String userId)
		{
			...
			if (!xml.isEmpty())
			{
                String str = xml.get(0).toString();
                //If there is no context in the submission xml, add it and commit the changes back to the table.
                if (str.contains("context=\"\"")) {

                    String contextSql = "select CONTEXT from " + m_assignmentsTableName + " where ASSIGNMENT_ID = ?";
                    Object contextField[] = new Object[1];
                    contextField[0] = assignmentId;

                    List contextList = m_sql.dbRead(contextSql, contextField, null);

                    if (!contextList.isEmpty()) {
                        String siteId = contextList.get(0).toString();

                        str = str.replace("context=\"\"", "context=\"" + siteId + "\"");

                        xml.set(0, str);

                        Object updateFields[] = new Object[3];
                        updateFields[0] = str;
                        updateFields[1] = assignmentId;
                        updateFields[2] = userId;

                        String updateSql = "update " + m_submissionsTableName + " set xml = ? where (" + SUBMISSION_FIELDS[0] + " = ? AND " + SUBMISSION_FIELDS[1] + "= ?)";

                        m_sql.dbWrite(updateSql, updateFields);
                    }
                }
		...
		private int getSubmissionsCountWhere(String queryType, String assignmentRef) {
			...
			try {
				...
				List<User> allowedUsers = AssignmentService.allowAddSubmissionUsers(a.getReference());
