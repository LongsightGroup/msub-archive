...
public class ContainerLogin extends HttpServlet
{
	...
	private boolean useRemoteUserHeader = false;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
	{
		...
		try
		{
			...
			if (remoteUser == null && useRemoteUserHeader)
			{
				remoteUser = req.getHeader("REMOTE_USER").trim();
			}
                        M_log.debug("remote user obtained through container authn: '" + remoteUser + "'");
			...
			{
				M_log.debug ("user logged in via UsageSessionService");
				// get the return URL
				...
				if (url == null)
				{
					M_log.error ("post-login return URL was not set, redirecting to portal gateway");
					url = portalPath;
				}
			...
			M_log.debug ("failed login via UserSessionService");
		}
		catch (AuthenticationException ex)
		{
			M_log.error("error authenticating: " + ex.getMessage(), ex);
		}
