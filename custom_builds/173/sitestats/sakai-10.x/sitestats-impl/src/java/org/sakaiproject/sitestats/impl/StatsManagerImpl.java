		...
		if(siteId == null){
			...
			HibernateCallback hcb = new HibernateCallback() {
				public Object doInHibernate(Session session) throws HibernateException, SQLException {
					...
					Criteria criteria = session.createCriteria(SiteActivityImpl.class)
					...
					if(events != null && events.size() > 0)
						...
						criteria.add(Restrictions.in("eventId", M_ers.getEventIds()));
					...
					if(iDate != null) {
						if(fDate != null){
							...
							c.add(Calendar.DAY_OF_YEAR, 1);
							Date fDate2 = c.getTime();
							criteria.add(Restrictions.between("date", iDate, fDate2));
						} else {
							criteria.add(Restrictions.ge("date", iDate));
						}
					}
