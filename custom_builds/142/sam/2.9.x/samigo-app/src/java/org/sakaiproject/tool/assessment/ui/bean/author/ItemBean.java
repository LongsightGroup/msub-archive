...
// need to trim, 'cuz in  mozilla, the field is printed as ^M , a new line char. 
	...
	//cannot add more than 26 matching pairs because of the size limit on the choice_label resource
	else if(list.size() >= 26) {
		FacesContext context=FacesContext.getCurrentInstance();
	    ResourceLoader rb = new ResourceLoader("org.sakaiproject.tool.assessment.bundle.AuthorMessages");
	    context.addMessage(null,new FacesMessage(rb.getString("match_outofbound_error")));
	    return true;
	}
	return false;
    }
