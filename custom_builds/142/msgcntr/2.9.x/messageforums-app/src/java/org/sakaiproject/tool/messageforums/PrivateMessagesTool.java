/**********************************************************************************
 * $URL: https://svn.rsmart.com/svn/vendor/branches/sakai/rsmart-cle/msgcntr/messageforums-app/src/java/org/sakaiproject/tool/messageforums/PrivateMessagesTool.java $
 * $Id: PrivateMessagesTool.java 36061 2012-06-08 22:09:36Z ktakacs $
 ***********************************************************************************
 *
  ...
  { 
      ...
      if (totalComposeToList != null) {
          ...
          for (Iterator i = totalComposeToList.iterator(); i.hasNext();) {
              ...
              if ((isInstructor() || item.isViewable() || isEmailPermit()) && isFilteredRecipient(item)) {
...
//    courseMemberMap = membershipManager.getAllCourseMembers(true, true, true);
  ...
  /**
   * @param MembershipItem item
   * @return boolean
   * 
   * Filter recipients based on the sakai property messages.[site type].[current user role].allowed.recipient.role
   */
	public boolean isFilteredRecipient(MembershipItem item) {
		...
		try{
			...
			String userRole = AuthzGroupService.getUserRole(SessionManager.getCurrentSessionUserId(), "/site/" + ToolManager.getCurrentPlacement().getContext());
			...
			allowedRecipients = ServerConfigurationService.getString("messages." + site.getType().toLowerCase() + "." + userRole.toLowerCase().replace(" ", "") + ".allowed.recipient.role");
			if (allowedRecipients != "") {
				if (item.getType() != 2 || // filter out non role types
					// next filter on role type set by messages.allowed.recipient.role
					(item.getType() == 2 && !allowedRecipients.equalsIgnoreCase(item.getRole().getId())) 
					) {
				// filter this recipient
				return false;
				}
			}
			
		} catch (IdUnusedException e) {
			LOG.debug("Can't find site type. Bogus site id = " + ToolManager.getCurrentPlacement().getContext());
			e.printStackTrace();
		}
		// do not filter recipient
		return true;
	}
  ...
  public boolean isAtMain(){
    ...
    public String getAuthorString() 
    {
       ...
       try
       {
         ...
         if (ServerConfigurationService.getBoolean("msg.displayEid", true)) {
             if (user != null) {
                 authorString = user.getLastName() + ", " + user.getFirstName() + " (" + user.getDisplayId() + ")";
             }
         } else {
             if (user != null) {
                 authorString = user.getLastName() + ", " + user.getFirstName();
             }
         }
       }
       catch(Exception e)
