/**********************************************************************************
 * $URL: https://svn.rsmart.com/svn/vendor/branches/sakai/rsmart-cle/msgcntr/messageforums-app/src/java/org/sakaiproject/tool/messageforums/DiscussionForumTool.java $
 * $Id: DiscussionForumTool.java 29769 2011-08-17 16:51:13Z ktakacs $
 ***********************************************************************************
 *
...
//  TODO:
  ...
  private DiscussionForum saveForumSettings(boolean draft)
  {
    ...
    String extendDescription = forum.getExtendedDescription();
    extendDescription = extendDescription.replace("<br type=\"_moz\" />", "");
    forum.setExtendedDescription(FormattedText.processFormattedText(extendDescription, alertMsg));
  ...
  private String saveTopicSettings(boolean draft)
  {
	...
	boolean synopticUpdate = isNew ? false : permissionsUpdated;
    	...
    	StringBuilder alertMsg = new StringBuilder();
           //remove the bad <br type="_moz" />
        String extendDescription = topic.getExtendedDescription();
        extendDescription = extendDescription.replace("<br type=\"_moz\" />", "");
    	...
    	while (iter.hasNext())
    	{
    		...
    		if (message != null)
    		{
    			...
    			if(isRead || (isNewResponse && decoMsg.getIsOwn()))
    			{
                    ...
                    if (getGradebookService() != null) {
                        if (isInstructor()) {
                            decoMsg.setAuthorCanBeGraded(getGradebookService().canBeGraded(decoMsg.getMessage().getCreatedBy(), getSiteId()));
                        }
                    }
...
//	// get/add the gradebook assignment associated with the topic
    ...
    public boolean getGradeable(){
        if(getGradebookService() != null){
            gradeable = getGradebookService().canBeGraded(selectedMessage.getMessage().getCreatedBy(), getSiteId());
        }
        return gradeable;
    }
    ...
    private boolean saveTopicsForGroups(boolean draft) {
        ...
        ArrayList attachmentsTemplate = new ArrayList(attachments);
