			...
			if (forward) {
				...
				if (existingSite != null) {
					...
					if (select_import_tools(params, state)) {
						...
						List existingTools = originalToolIds((List<String>) state.getAttribute(STATE_TOOL_REGISTRATION_SELECTED_LIST), state);
			...
			else if (isMultipleInstancesAllowed(findOriginalToolId(state, id)) && (idSelected != null && !idSelected.contains(id) || idSelected == null))
			{
				...
				if (attributes != null)
				{
					for(Iterator<String> e = attributes.keySet().iterator(); e.hasNext();)
					{
						...
						{
							// validate url for web content
							if (id.equals(WEB_CONTENT_TOOL_ID) && attribute.equals(WEB_CONTENT_TOOL_SOURCE_CONFIG) && 
									!FormattedText.validateURL(attributeInput)) {
