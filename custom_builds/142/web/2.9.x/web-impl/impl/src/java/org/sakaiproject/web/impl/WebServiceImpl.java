 ...
 * $Id: WebServiceImpl.java 37012 2012-09-06 21:17:06Z jbush $
 ***********************************************************************************
 *
...
public class WebServiceImpl implements WebService, EntityTransferrer, EntityTransferrerRefMigrator
{
	...
	public Map<String, String> transferCopyEntitiesRefMigrator(String fromContext, String toContext, List<String> ids)
	{
		...
		{				
            ...
            transversalMap.put(fromSite.getId(), toSite.getId());
	...
	public Map<String, String> transferCopyEntitiesRefMigrator(String fromContext, String toContext, List<String> ids, boolean cleanup)
	{
		...
		try
		{
			if(cleanup == true)
			{
                ...
                transversalMap.put(fromContext, toContext);
    ...
    public void updateEntityReferences(String toContext, Map<String, String> transversalMap)
    {
        if(transversalMap != null && transversalMap.size() > 0){
            ...
            try {
                Site destinationSite = SiteService.getSite(toContext);

                Collection<ToolConfiguration> webTools = destinationSite.getTools(myToolIds());

                if (webTools != null && !webTools.isEmpty()) {
                    for (Iterator it = webTools.iterator(); it.hasNext(); ) {
                        ToolConfiguration toolConfig = (ToolConfiguration) it.next();

                        boolean updated = false;

                        if(toolConfig.getConfig() != null){
                            String source = toolConfig.getConfig().getProperty("source", null);
                            ...
                            if(source != null && !"".equals(source)){
                                ...
                                while(entryItr.hasNext()){
                                    ...
                                    if(source.contains(fromContextRef)){
                                        source = source.replace(fromContextRef, entry.getValue());
                                        updated = true;
                                    }

                                    if(updated){
                                        toolConfig.getPlacementConfig().setProperty("source", source);
                                        toolConfig.save();
                                    }
                                }
                            }
                ...
                M_log.error("WebServiceImpl updateEntityReferences error " + e, e);
            }
        }
    }
}
