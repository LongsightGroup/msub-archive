...
public class RosterBean extends EnrollmentTableBean implements Serializable, Paging {
	...
	private List<GradableObjectColumn> gradableObjectColumns;	// Needed to build table columns
	...
	protected void init() {
		...
		gradableObjectColumns = new ArrayList<GradableObjectColumn>();
				...
				if(selectedCategoryUid == null || selectedCategoryUid.equals(cat.getId().toString())){
					...
					if (assignments != null && !assignments.isEmpty()){
						for (Iterator assignmentsIter = assignments.iterator(); assignmentsIter.hasNext();){
                            GradableObjectColumn categoryAssignment = new GradableObjectColumn((GradableObject)assignmentsIter.next());
                            categoryAssignment.setCategoryName(categoryColumn.getName());
                            gradableObjectColumns.add(categoryAssignment);						
                        }
        ...
        if (!isEnrollmentSort()) {
			...
			for(Iterator iter = gradeRecords.iterator(); iter.hasNext();) {
				...
				if(getColumnHeader(agr.getGradableObject()).equals(sortColumn)) {
                    if (studentIdEnrRecMap.get(agr.getStudentId()) != null){
...
*/		                
	...
	public List<GradableObjectColumn> getGradableObjectColumns() {
		return gradableObjectColumns;
	}
	public void setGradableObjectColumns(List<GradableObjectColumn> gradableObjectColumns) {
