...
public abstract class EnrollmentTableBean
	...
	private Map transformToOrderedEnrollmentMapWithFunction(Map enrRecFunctionMap) {
        ...
        for (Iterator iter = enrollmentList.iterator(); iter.hasNext(); ) {
            ...
            if(getGradebookBean().canViewOwnGrades(enr.getUser().getUserUid(), getGradebookUid())){
	...
	public boolean isEnrollmentSort() {
      ...
      return columnSortMap.get(sortColumn) != null;
   }
   ...
   public void addSortingColumn(String key, Comparator<EnrollmentRecord> comparator) {
      columnSortMap.put(key, comparator);
   }
}
