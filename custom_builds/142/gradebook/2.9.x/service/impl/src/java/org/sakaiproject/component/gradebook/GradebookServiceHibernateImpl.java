...
public class GradebookServiceHibernateImpl extends BaseHibernateManager implements GradebookService {
  ...
  public List<CourseGradeRecord> getGradeRecords(String gradbookUid, Collection studentUids) {
      return getGradeRecords(gradbookUid, studentUids, null);
   }

   public List<CourseGradeRecord> getGradeRecords(String gradbookUid, Collection studentUids, Date cutoffDate) {
      Gradebook gradebook = getGradebook(gradbookUid);
      List<org.sakaiproject.service.gradebook.shared.Assignment> assignments = getAssignments(gradbookUid);

      if (studentUids == null) {
         studentUids = getAllStudentUids(gradbookUid);
      }

      List assignmentsUsed = new ArrayList();
      List<AssignmentGradeRecord> gradeRecords = getAllAssignmentGradeRecords(gradebook.getId(), studentUids);
      List<AssignmentGradeRecord> gradeRecordsUse = new ArrayList(gradeRecords);

      for (org.sakaiproject.service.gradebook.shared.Assignment assignment : assignments) {
         if (cutoffDate == null ||
            (assignment.getDueDate() != null && assignment.getDueDate().before(cutoffDate))) {
            Assignment newAssignment = new Assignment(gradebook, assignment.getName(),
               assignment.getPoints(), assignment.getDueDate());
            newAssignment.setId(assignment.getId());
            assignmentsUsed.add(newAssignment);
            for (AssignmentGradeRecord agr : gradeRecords) {
               if (agr.getGradableObject().getId().equals(assignment.getId()) &&
                  agr.getGradableObject() instanceof Assignment) {
                  newAssignment.setCategory(((Assignment)agr.getGradableObject()).getCategory());
               }
            }
         }
         else {
            // remove this one from gradeRecords
            for (AssignmentGradeRecord agr : gradeRecords) {
               if (agr.getGradableObject().getId().equals(assignment.getId())) {
                  gradeRecordsUse.remove(agr);
               }
            }
         }
      }
      ...
      addToGradeRecordMap(gradeRecordMap, gradeRecordsUse);

      return getPointsEarnedCourseGradeRecords(courseGrade, studentUids, assignmentsUsed, gradeRecordMap);
   }
