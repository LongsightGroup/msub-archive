		...
		RWikiCurrentObjectDao, ObjectProxy
{
			...
			final String user, final String realm)
	{
		...
		for (int i = 0; i < criterias.length; i++)
		{
			...
			{
				if ("db2".equals(SqlService.getVendor())) {
					expression.append(" or lower(varchar(c.content)) like ? ");
			    }                
                else {
		...
		HibernateCallback callback = new HibernateCallback()
		{
			public Object doInHibernate(Session session)
					throws HibernateException
			{
				String searchClause = "          lower(c.content) like ? ";

				if ("db2".equals(SqlService.getVendor())) {
					searchClause = "          lower(varchar(c.content)) like ? ";
				}
				return session
						.createQuery(
								"select distinct r "
		...
		HibernateCallback callback = new HibernateCallback()
		{
					...
					throws HibernateException
			{
				String search = globalParentPageName.replaceAll("([A%_])",
						"A$1");
            String vendor = org.sakaiproject.db.cover.SqlService.getVendor();
            String returnString = "from RWikiCurrentObjectImpl as r " +
                    "where r.name like ";
            ...
            returnString += " escape 'A' " +
                    "order by name asc";
				return session.createQuery(
						returnString).setParameter(0, search,
	...
	public RWikiObject findLastRWikiSubPage(final String globalParentPageName)
	{
		HibernateCallback callback = new HibernateCallback()
		{
            ...
            returnString += "escape 'A' "
						+ "order by name desc";
            return session.createQuery(returnString).setParameter(0, search,
						Hibernate.STRING).list();
			}
		};
