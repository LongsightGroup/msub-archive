...
public class AnnouncementAction extends PagedResourceActionII
{
   ...
   /*
			...
			SessionState sstate)
	{
		...
		mergedAnnouncementList.loadChannelsFromDelimitedString(isOnWorkspaceTab(), new EntryProvider(true), StringUtils
				.trimToEmpty(SessionManager.getCurrentSessionUserId()), mergedAnnouncementList
				.getChannelReferenceArrayFromDelimitedString(state.getChannelId(), portlet.getPortletConfig().getInitParameter(
						getPortletConfigParameterNameForLoadOnly(portlet))), m_securityService.isSuperUser(), ToolManager
	...
	private boolean canViewHidden(AnnouncementMessage msg, String siteId)
	{
        ...
        if (SiteService.allowUpdateSite(ToolManager.getCurrentPlacement().getContext())) {
		  return m_securityService.unlock(AnnouncementService.SECURE_ANNC_READ_DRAFT, msg.getReference())
			...
			AnnouncementActionState state, SessionState sstate)
	{
		...
		else if (state.getStatus().equals("goToReviseAnnouncement"))
		{
			...
			// get the notification setting if it is future announcement or draft
			if(releaseDate.after(TimeService.newTime()) || edit.getHeader().getDraft()){
				context.put("noti", edit.getProperties().getProperty("notificationLevel"));
			}
	...
	protected void readAnnouncementForm(RunData rundata, Context context, boolean checkForm)
	{
      ...
      SimpleDateFormat dateFormatter = new SimpleDateFormat("MM/dd/yyyy hh:mm a");
		...
		if (use_start_date && SPECIFY_DATES.equals(specify))
		{
         ...
         String s = releaseDate.toStringLocal();
                s=  releaseDate.toStringLocalFull();
                s = dateFormatter.format(new Date(releaseDate.getTime()));
...
//				header.setDraft(!post);
				...
				if(placement!= null && ("MOTD".equals(placement.getTitle()) && placement.getId().contains("admin")) || (placement.getContext() != null && "!admin".equals(placement.getContext())))
				{
				...
				}else  if(msg.getAnnouncementHeaderEdit().getDraft()){
					//If assignment is draft Hold selected notification value and update notification history with selected value.
					msg.getPropertiesEdit().addProperty("notificationLevel", notification);
