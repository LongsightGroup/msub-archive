    ...
    <html xmlns="http://www.w3.org/1999/xhtml" lang="en" xml:lang="en">
      ...
      <script type="text/javascript">
        $(document).ready(function() {
            ...
            var blockDivs = document.forms[0].elements['assessmentSettingsAction:blockDivs'].value;
            var expandAll = false;
            if (blockDivs != "") {
            	for (i=0 ; i < divElements.length; i++)
	            {
	            	if(divElements[i].id.indexOf("__hide_division_assessmentSettingsAction") >=0)
	                {
	            		if(divElements[i].id.indexOf(blockDivs) >=0)
		               		break;
	            		count++;
	                }
	            }
            	try{
            		if(blockDivs.split(";").length > 1){
            			//this means multiple divs are open, this can only happen with expand all
            			expandAll = true;
            		}
            	}catch(e){}
            }
            ...
            $("#jqueryui-accordion").accordion({ autoHeight: false,collapsible: true, active: count });
            if(expandAll){
	        	expandAccordion('<%= org.sakaiproject.util.Web.escapeJavascript(thisId)%>');
	        }
...
{  
   ...
   for (i=0 ; i < blockElements.length; i++)
   {
      ...
      if(divisionNo.indexOf("__hide_division_assessmentSettingsAction") >=0 && $(blockElements[i]).is(":visible"))
      { 
         //alert("divisionNo=" + divisionNo);
...
<f:verbatim></div></f:verbatim>
        ...
        <script type="text/JavaScript">showHideReleaseGroups();retainHideUnhideStatus('none');</script>
      </body>
    </html>
  </f:view>
