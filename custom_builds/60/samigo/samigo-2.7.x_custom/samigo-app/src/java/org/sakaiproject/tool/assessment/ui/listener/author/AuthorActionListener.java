    ...
    implements ActionListener
{
  ...
  public void processAction(ActionEvent ae) throws AbortProcessingException
  {
    ...
    author.setShowTemplateList(Boolean.parseBoolean(showAssessmentTypes));
  ...
  public void prepareAllPublishedAssessmentsList(AuthorBean author, GradingService gradingService, PublishedAssessmentService publishedAssessmentService) {
	  ...
	  setInProgressAndSubmittedCount(publishedAssessmentList, inProgressCounts, submittedCounts);
	  setStatusBasedOnAvailability(publishedAssessmentList);
	  author.setPublishedAssessments(publishedAssessmentList);
	  prepareRetractWarningText(author, (ArrayList) publishedAssessmentList);  
  }

  public void prepareRetractWarningText(AuthorBean author, ArrayList publishedAssessmentList) {	  
	  boolean isAnyAssessmentRetractForEdit = false;
	  Iterator iter = publishedAssessmentList.iterator();
	  while (iter.hasNext()) {
  ...
  public void setStatusBasedOnAvailability(ArrayList<PublishedAssessmentFacade> list) {
	  for (int i = 0; i < list.size(); i++) {
		  PublishedAssessmentFacade p = (PublishedAssessmentFacade) list.get(i);
		  p.setActiveStatus(isActive(p));
	  }
  }

  public void setInProgressAndSubmittedCount(ArrayList<PublishedAssessmentFacade> list, HashMap<Long, Integer> inProgressCounts, HashMap<Long, Integer> submittedCounts) {
	  for (int i = 0; i < list.size(); i++) {
