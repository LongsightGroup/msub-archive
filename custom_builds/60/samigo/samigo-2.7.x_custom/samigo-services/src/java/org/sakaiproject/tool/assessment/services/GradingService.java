  ...
  public void storeGrades(AssessmentGradingIfc data, boolean regrade, PublishedAssessmentIfc pub,
    ...
    try {
      ...
      while(iter.hasNext())
      {
        ...
        totalAutoScoreCount += itemGrading.getAutoScore();
      }
      log.debug("****x4. "+(new Date()).getTime());
      
      //if there is a minimum score value, then make sure the auto score is at least the minimum
      if(minScore != null && totalAutoScoreCount < minScore){    	  
          iter = itemGradingSet.iterator();
          while(iter.hasNext())
          {
            ItemGradingIfc itemGrading = (ItemGradingIfc) iter.next();
           	itemGrading.setAutoScore( minScore / itemGradingSet.size());
          }
