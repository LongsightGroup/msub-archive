...
public class UsersAction extends PagedResourceActionII
{
	...
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData rundata)
	{
		...
		if (state.getAttribute("user.recaptcha-enabled") == null)
		{
			...
			state.setAttribute("user.recaptcha-enabled", enabled);
                ...
                if (state.getAttribute("user.portal.display.user.skin-enabled") == null)
                {
                        Boolean enabled = ServerConfigurationService.getBoolean("portal.display.user.skin", false);
                        state.setAttribute("user.portal.display.user.skin-enabled", enabled);
                }
    ...
    context.put("superUser", Boolean.valueOf(SecurityService.isSuperUser()));
		...
		if (value != null) context.put("valueType", value);
               ...
               context.put("showSkinProperty", ((Boolean)state.getAttribute("user.portal.display.user.skin-enabled")));
               if(((Boolean)state.getAttribute("user.portal.display.user.skin-enabled"))){
                       
                       value = (String) state.getAttribute("valueSkin");
                       if (value != null) context.put("valueSkin", value);
               }       
	...
	private String buildEditContext(SessionState state, Context context)
	{
		...
		context.put("incPw", state.getAttribute("include-password"));
               ...
               context.put("showSkinProperty", ((Boolean)state.getAttribute("user.portal.display.user.skin-enabled")));
               if(((Boolean)state.getAttribute("user.portal.display.user.skin-enabled")))
                       context.put("userSkin", getUserProperty(state, user.getId(), ResourceProperties.PROP_USER_SKIN));
	...
	public void doSave(RunData data, Context context)
	{
		...
		if (user == null)
		{
			user = (User) state.getAttribute("newuser");
               ...
               if(user != null && ((Boolean)state.getAttribute("user.portal.display.user.skin-enabled")))
               {
                       String skin = (String) state.getAttribute("valueSkin");
                       setUserProperty(state, user.getId(), ResourceProperties.PROP_USER_SKIN, skin);
               }
		...
		state.removeAttribute("valueType");
               if((Boolean)state.getAttribute("user.portal.display.user.skin-enabled"))
                       state.removeAttribute("valueSkin");
	...
	private boolean readUserForm(RunData data, SessionState state)
	{
        ...
        String pwcur = StringUtils.trimToNull(data.getParameters().getString("pwcur"));
               ...
               if((Boolean)state.getAttribute("user.portal.display.user.skin-enabled")){
                       ...
                       state.setAttribute("valueSkin", skin);
               }
      ...
      if (!SecurityService.isSuperUser()) {
       ...
       private String getUserProperty(SessionState state, String userId, String key) {
               String ret = "";
               try {
                       ...
                       Object[] params = new Object[]{key, userId};
                       addAlert(state, rb.getFormattedMessage("useact.getproperror", params));
               }

               return ret;
       }
       
       private void setUserProperty(SessionState state, String userId, String key, String value) {
               ...
               try {
                       ...
                       Object[] params = new Object[]{key, userId};
                       addAlert(state, rb.getFormattedMessage("useact.setproperror", params));
               }
       }
}
