...
public class SearchResultImpl implements SearchResult
{
	...
	public String getUrl()
	{
        ...
        {
                String url = doc.get(SearchService.FIELD_URL);
                int lastIndex = url.lastIndexOf("/");
                String path = url.substring(0, lastIndex);
                return path;
        }

        public String getResourcesPrettyPath() 
        {
               String path = getResourcesPath();
               path = path.substring(23 + getSiteId().length());
               path = path.replace("/", " > ");
               return path;
        }
	...
	{
                String tool = doc.get(SearchService.FIELD_TOOL);
                if (tool.equals("content")) {
                    tool = "resources";
                }
		return tool;
	...
	public String getSearchResult()
	{
		try
		{
			...
			String text = sb.toString();
                        //This should probably go into appendCleanString when index is built, but possibly too late for that
