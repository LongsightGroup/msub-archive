  ...
  <sakai:view title="#{msgs.pvt_repmsg_ALL}">
    ...
    <script type="text/javascript">
       function FCKeditor_OnComplete( editorInstance )
      {
             ...
             {
                     //fix the formatting
                     var innerHtmlJQObj = $j( oEditor.EditorDocument.body );
                     innerHtmlJQObj.children('br:first, br:nth-child(2), span').wrapAll('<div />');
             }
        }
    </script>
    <script type="text/javascript">
		...
		<h:panelGroup styleClass="shorttext">
		 ...
		 <h:panelGroup styleClass="shorttext" rendered= "#{PrivateMessagesTool.dispSendEmailOut}">
		   ...
		   <h:outputLabel for="send_email_out"><h:outputText value="#{msgs.pvt_send_as_email}"/></h:outputLabel>
			...
			<sakai:inputRichText rows="#{ForumTool.editorRows}" cols="120" id="pvt_forward_body" value="#{PrivateMessagesTool.forwardBody}">
...
<%--													  <h:outputLink value="#{eachAttach.attachmentUrl}" target="_blank">
      ...
      <sakai:button_bar>
        ...
        <sakai:button_bar_item action="#{PrivateMessagesTool.processPvtMsgPreviewReplyAll}" value="#{msgs.pvt_preview}" accesskey="p"  styleClass="active" />
        <sakai:button_bar_item action="#{PrivateMessagesTool.processPvtMsgReplyAllSaveDraft}" value="#{msgs.pvt_savedraft }" />
