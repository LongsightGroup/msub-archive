...
//  TODO:
  ...
  private Date getDateNotMidnight(Date date){
	  if(date != null){
  ...
  private DiscussionForum saveForumSettings(boolean draft)
  {
    ...
    //first make sure the available dates do not equal 12:00AM (some issue with this never firing)
    if(forum.getOpenDate() != null){
    	forum.setOpenDate(getDateNotMidnight(forum.getOpenDate()));
    }
    if(forum.getCloseDate() != null){
    	forum.setCloseDate(getDateNotMidnight(forum.getCloseDate()));
    }
    if(!isNew){
  ...
  private String saveTopicSettings(boolean draft)
  {
    ...
    if (selectedTopic != null)
    {
      ...
      if (selectedForum != null)
      {
    	...
    	//first make sure the available dates do not equal 12:00AM (some issue with this never firing)
        if(topic.getOpenDate() != null){
        	topic.setOpenDate(getDateNotMidnight(topic.getOpenDate()));
        }
        if(topic.getCloseDate() != null){
        	topic.setCloseDate(getDateNotMidnight(topic.getCloseDate()));
        }
	if(!isNew){
