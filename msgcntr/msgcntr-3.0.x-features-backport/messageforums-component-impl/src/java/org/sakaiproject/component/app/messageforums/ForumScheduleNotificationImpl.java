...
public class ForumScheduleNotificationImpl implements ForumScheduleNotification {
    ...
    private void scheduleAvailability(String id, boolean availabilityRestricted, Date openDate, Date closeDate){
    	...
    	LOG.info("ForumScheduleNotificationImpl.scheduleAvailability: id: " + id + ", availabilityRestricted: " + availabilityRestricted + ", openDate: " + openDate + ", closeDate: " + closeDate);
    	try{
    	...
    	if (fdi != null && fdi.length > 0)
    	{
    		...
    		{
    			LOG.info("ForumScheduleNotificationImpl.scheduleAvailability: deleting delayed invocation: " + d.uuid);
    	...
    	if (availabilityRestricted)
    	{
    		...
    		{
    			LOG.info("ForumScheduleNotificationImpl.scheduleAvailability: scheduling openTime");
    			scheduledInvocationManager.createDelayedInvocation(openTime,
    					...
    					id);
    			LOG.info("ForumScheduleNotificationImpl.scheduleAvailability: scheduling complete");
    		}
    		if(closeTime != null && closeTime.after(timeService.newTime())){
    			LOG.info("ForumScheduleNotificationImpl.scheduleAvailability: scheduling closeTime");
    ...
    public void execute(String opaqueContext){
    	...
    	if(opaqueContext.startsWith(AREA_PREFIX)){
    		...
    		logInformation(area.getId(), "Area", area.getOpenDate(), area.getCloseDate(), makeAvailable, madeChange);
    		if(madeChange){
    			//save area and update synoptic counts
    			areaManager.saveArea(area);    			
    			LOG.info("ForumScheduleNotificationImpl 1");
    			SynopticMsgcntrManagerCover.resetAllUsersSynopticInfoInSite(siteId);
    			LOG.info("ForumScheduleNotificationImpl 2");
    		}
    		...
    		logInformation(forumId, "Forum", forum.getOpenDate(), forum.getCloseDate(), makeAvailable, madeChange);
    		if(madeChange){
    			//save forum and update synoptic counts
    			String siteId = forumManager.getContextForForumById(forumId);
    			LOG.info("ForumScheduleNotificationImpl " + siteId);
    			...
    			LOG.info("ForumScheduleNotificationImpl 3");
    			...
    			LOG.info("ForumScheduleNotificationImpl 4");
    		}
    		...
    		logInformation(topicId, "Topic", topic.getOpenDate(), topic.getCloseDate(), makeAvailable, madeChange);
    		if(madeChange){
    			//save forum and update synoptic counts
    ...
    private void logInformation(Long id, String type, Date openDate, Date closeDate, boolean makeAvailable, boolean madeChange){
    	LOG.info("ForumScheduleNotificationImpl: type: " + type + ", id: " + id + ", makeAvailable: " + makeAvailable + ", madeChange: " + madeChange);
    	if(openDate != null){
    		LOG.info("openDate: " + dateFormat.format(openDate));
    	}
    	if(closeDate != null){
    		LOG.info("closeDate: " + dateFormat.format(closeDate));
    	}
    ...
    public boolean makeAvailableHelper(boolean availabilityRestricted, Date openDate, Date closeDate){
		...
		if(availabilityRestricted){
			...
			if(openTime != null){
				LOG.info("openTime: " + openTime.getDisplay());
			}
			if(closeTime != null){
				LOG.info("closeTime: " + closeTime.getDisplay());
			}
			LOG.info("currentTime: " + timeService.newTime().getDisplay());
		}
		LOG.info("makeAvailable: " + makeAvailable);
