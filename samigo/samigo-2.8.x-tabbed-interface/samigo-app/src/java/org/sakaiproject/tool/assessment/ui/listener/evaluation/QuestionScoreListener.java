			...
			while (iter.hasNext()) {
				...
				while (iter2.hasNext()) {
					...
					//SAM-755-"checkmark" indicates right, add "X" to indicate wrong
                       if (bean.getTypeId().equals("8")) {
                           // get result for Fill In the Blank question
                           if (delegate.getFIBResult(gdata, fibmap, item, publishedAnswerHash)) {
                       ...
                       //Manual fix, no type 15 CALCULATED_QUESTION in 2.8
                       else if (bean.getTypeId().equals("11")) {
