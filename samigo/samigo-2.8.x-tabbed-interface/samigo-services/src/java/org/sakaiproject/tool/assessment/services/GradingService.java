  ...
  */
          ...
          if(!(MathUtils.equalsIncludingNaN(entry.getValue()[0], entry.getValue()[1], 0.0001))){
                  ...
                  while(iter.hasNext()) {
                          ...
                          if(itemGrading.getPublishedItemId().equals(entry.getKey())) {
                                  itemGrading.setAutoScore(Float.valueOf(0));
                                  log.debug ("MCMS setAutoScore=0 ON itemGrading=" + itemGrading.getItemGradingId() + ";itemId=" + itemGrading.getPublishedItemId());
                          }
                  }
          }
      }
  ...
  public float getAnswerScore(ItemGradingIfc data, HashMap publishedAnswerHash)
  {
    if (log.isDebugEnabled()) {
      try {
        log.debug("getAnswerScore publishedAnswerId=" + data.getPublishedAnswerId());

        Iterator i = publishedAnswerHash.keySet().iterator();
        while (i.hasNext()) {
          Long l = (Long) i.next();
          AnswerIfc a = (AnswerIfc) publishedAnswerHash.get(l);
          System.out.println("getAnswerScore publishedAnswerHash key=" + l + ";id=" + a.getId() + ";score=" + a.getScore() + ";correct=" + a.getIsCorrect());  
        }
      }
      catch (Exception e) {
        log.debug("getAnswerScore exception when trying to print hashmap of publishedAnswerHash");
      }
    }
    ...
    if (answer == null || answer.getScore() == null) {
        if (log.isDebugEnabled()) {
          log.debug("getAnswerScore answer is null or answer score is null");
        }
    ...
    {
        if (log.isDebugEnabled()) {
          log.debug("getAnswerScore answer.getIsCorrect is null or not a boolean");
        }
    ...
    if (log.isDebugEnabled()) {
      log.debug("getAnswerScore returning score for agent=" + data.getAgentId() + ";score=" + answer.getScore().floatValue());
    }
