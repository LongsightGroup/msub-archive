/**
 * $Id: SetupEvalBean.java 61559 2009-07-01 20:38:40Z jimeng@umich.edu $
 * $URL: https://source.sakaiproject.org/contrib/evaluation/trunk/tool/src/java/org/sakaiproject/evaluation/tool/SetupEvalBean.java $
...
public class SetupEvalBean {
    ...
    /**
     * This is set to the type of email template when resetting the evaluation to use default
     * templates
     */
    ...
    /**
     * selection value constants to populate the evaluation selection settings
     */
    public String selectionInstructors;
    ...
    /**
     * sets the Locale on the date formatter correctly
     */
    public void init() {
    ...
    /**
     * Handles saving and assigning email templates to an evaluation, can just assign the email
     * template if the emailTemplateId is set or will save and assign the one in the locator
     */
    ...
    public String completeSettingsAction() {
        ...
        // SELECTION SETTINGS
        evaluationBeanLocator.selectionSettings = setSelectionOptions();
    ...
    /**
     * Complete the creation process for an evaluation (view all the current settings and
     * assignments and create eval/assignments), this will save the node/group assignments that were
     * submitted and reconcile this list with the previous set of nodes/groups and remove any that
     * are now missing before adding the new ones
     */
    public String completeConfirmAction() {
        ...
        if (EvalConstants.EVALUATION_STATE_PARTIAL.equals(eval.getState())) {
            ...
            // save the new evaluation state (moving from partial), set to true should fix up the
            // state automatically
            ...
            List<EvalAssignHierarchy> assignedHierList = evaluationSetupService.setEvalAssignments(
                    evaluationId, selectedHierarchyNodeIDs, selectedGroupIDs, false);
            ...
            if (EvalUtils.checkStateAfter(eval.getState(), EvalConstants.EVALUATION_STATE_ACTIVE,
                    false)) {
                throw new IllegalStateException(
            ...
            if (EvalUtils.checkStateAfter(eval.getState(), EvalConstants.EVALUATION_STATE_INQUEUE,
                    false)) {
                append = true; // can only append after active
            }

            // save all the assignments (hierarchy and group)
            evaluationSetupService.setEvalAssignments(evaluationId, selectedHierarchyNodeIDs,
                    selectedGroupIDs, append);
        }
        return "controlEvals";
    }
    ...
    /**
     * Create {@link Map} object to inject into Eval and set Selection Options eg.for
     * {@link EvalAssignGroup.SELECTION_TYPE_INSTRUCTOR} and
